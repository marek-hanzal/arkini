...
Tile interaction uses one Canvas-wide Motion actor system currently shared by the rendered Board and Toolbar. The actor system is storage-surface agnostic, but an active Inventory surface and its cross-surface interaction flow are still open work rather than implemented UI. Rendered storage surfaces expose stable slot anchors only; `TileActorLayer` renders exactly one visible stable DOM actor for each live runtime item identity, independent of its current storage surface. Arkini owns actor phase, operation generation, source/target semantics, z-order, topmost `slot | surface | outside` hit testing, engine command selection, and reconciliation targets. Motion receives that Arkini-authored state and owns the physical drag gesture, frame batching, scale, opacity, velocity continuity, and spring interpolation. Pointer press is visually inert: it only arms gesture arbitration and preserves the current stable or hover pose. Once Motion crosses the drag threshold, the actor uses a Motion-owned `left/top` anchor shell, an exact `x/y` pointer drag shell, and a nested short pickup-correction shell that eases an off-center grab toward the actor center without filtering pointer movement. Settlement folds both local offsets into the current visual pose before resetting them, so pointer tracking and placement transforms never compete or snap. The centered inner tile body rests at scale `0.8`; hover, semantic target emphasis, and active drag remain restrained around that baseline. Every release invokes one public atomic engine drop command; the UI never infers move, merge, consume, swap, stacking, or storage eligibility. The Board command resolves empty-slot move, non-mergeable occupied swap, authored directional consume merge, same-slot ignore, and explicit reject. Merge outcomes expose exact previous/current source and target identities; source approach is followed by independently completing return, impact, or exit phases. Removed gameplay identities may remain in the actor layer only while the active presentation generation protects them, and synchronous engine publication cannot create a one-frame unmount between command commit and outcome settlement. No ghost, clone, screenshot, hidden canonical tile, per-surface actor tree, manual RAF drag runtime, or direct WAAPI path exists.
