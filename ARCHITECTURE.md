...
Editor item estimates use one explicit three-layer boundary. The canonical engine owns every gameplay transition and remains the sole authority for inputs, jobs, reservations, charges, lifecycle, merges, output resolution, and the resulting immutable runtime snapshot. Planner strategies own only decision algorithms over those transitions: producer expansion explores bounded producer worlds, best-first maintains a runtime frontier, and constructive search maintains goals, speculative snapshots, and backtracking. No strategy may mutate the live game, publish editor DTOs, or replace an engine rule with a parallel economic implementation. Persistent estimate data under the renderer's IndexedDB origin is only a best-effort derived cache keyed by project revision and planner revision; it can be pruned or rebuilt and is never canonical editor-project persistence.

`Planner` is the sole public orchestration capability. It owns one immutable planner kernel for the authored config, creates a fresh Effect-native planning session for every estimate, invokes exactly one configured root strategy, validates the returned engine witness, and turns that concrete trace into expected economics. The session provides the shared global budget, current-strategy identity, invocation diagnostics, and recursive `solveSubgoalFx` routing. Simulated worlds remain immutable branch-local values; session services coordinate search work but never become a second gameplay store. Delegation depth belongs exclusively to the branch-local `PlannerCurrentStrategyFx` invocation context, while `PlannerProblem` owns only the immutable world plus a normalized active-first goal agenda.

The production root is `EditorPlannerStrategy`. For a bounded reachable target depth it first gives producer expansion one attempt over the current immutable problem. If that attempt is inconclusive or producer expansion is outside its configured depth, the root's local deterministic selector chooses constructive or best-first from the active goal's charge requirement, stochastic routes, acquisition depth, root shape, and current strategy path. An inconclusive constructive attempt receives one bounded best-first fallback over the original immutable problem; best-first is otherwise terminal for that root invocation. A child strategy that discovers a new subgoal delegates through `PlannerSessionFx.solveSubgoalFx`, which re-enters this same editor root under a deeper `PlannerCurrentStrategyFx` context, so the root may make a different deterministic selection for the subproblem. Callers never carry a parallel strategy list or fallback plan, and scheduling/concurrency cannot change global budgets, engine validity, or normalized planner results.
