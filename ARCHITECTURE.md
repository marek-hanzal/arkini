...
For every requested item quantity, the estimator evaluates each complete route recursively to authored roots. Required sibling branches are deliberately serialized by addition, even when gameplay could run them concurrently. Route selection compares the complete conservative duration and then a stable route identity; it never performs state-space search or critical-path scheduling. Consumables scale with required runs, while reusable and ongoing prerequisites retain their explicit requirement meaning and shared one-time acquisition is deduplicated by canonical fact identity. Cyclic or unreachable alternatives produce path diagnostics without poisoning a valid alternative route.

The editor worker, UI, and MCP projection consume only the immutable estimate result. The renderer keeps at most one process-lifetime in-memory batch for the current project revision; a newer authored snapshot interrupts and replaces it. This batch is disposable derived data, never canonical editor-project persistence. Presentation explains the selected route tree, conservative serialized duration, consumables, one-time prerequisites, explicit static-analysis limitations, and rejected alternatives, but it must not claim an engine-valid witness or exact player wall-clock time. Authored random output occurrences retain the editor economics contract and are evaluated independently by expected yield. Scope, distance, concrete placement, conditional runtime changes, and charge-renewal timing stay outside the closed item-fact vocabulary and are surfaced as limitations instead of being silently simulated.
