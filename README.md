...
Client-only offline merge-game prototype. Plain Vite + React SPA, static-host friendly, no SSR, no server runtime, no HTTP API. Live gameplay state belongs to `RuntimeGameEngineAdapter` and `GameRuntimeStore`; persistence will wrap the runtime later instead of dictating its shape. Static gameplay rules are authored as JSON fragments and packed with PNG assets into a gzip-compressed Arkini binary pack. `CONFIG.MD` documents config shape, `GAME.MD` documents intended gameplay semantics, and `SELECTOR.MD` documents selector matching so nobody has to reverse-engineer `anyOf`/`allOf` every time a grant sneezes.
...
Build the default `game/arkini.game.arkpack` from every JSON and PNG file below
`game/arkini`:
...
The v1 pack step intentionally performs no schema validation yet. It merges the JSON authoring
fragments, stores PNGs as raw resources, writes the v0-compatible binary envelope, and gzip-compresses
the final `.arkpack` transport.
