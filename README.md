...
XState is used only where a real phased workflow still earns it: generic drag/drop phases, TileEngine motion handoff, delayed merge hints, and tiny reset workflows. SQLite remains the durable game state, React Query remains the read/cache layer, and Effect remains the persistence/domain runtime. Plain React hooks own simple UI toggles, feedback timers, and FIFO scheduling because not every boolean deserves a statechart pension.
