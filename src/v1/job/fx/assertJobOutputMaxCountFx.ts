...
/** Rejects a newly active job when its worst-case output would overbook maxCount. */
export const assertJobOutputMaxCountFx = Effect.fn("assertJobOutputMaxCountFx")(function* ({
	job,
	runtime,
}: assertJobOutputMaxCountFx.Props) {
	const candidateQuantities = yield* readJobMaximumOutputQuantitiesFx({
		job,
		runtime,
	});

	for (const itemId of candidateQuantities.keys()) {
