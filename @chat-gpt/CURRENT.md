...
- Info is the first real capability vertical slice. Capability availability is read through one exact live runtime identity, so the hover strip now renders only the implemented Info action; Status, Lines, and Effects resources remain packaged but are not dead controls. Clicking Info freezes the exact runtime ID and originating actor, closes hover, and opens one Canvas-local viewport-constrained `TileWorkspace` over an unchanged Board/Toolbar. The workspace owns one generation-safe `closed | entering | open | exiting` lifecycle, backdrop dismissal, capture-phase Escape ownership, focus containment/restoration, stale-target close, and local Motion only. The Info projection exposes only title, category subtitle, description, and resolved authored source/composite resource URLs. Its sparse body reserves most of the stable modal footprint for a large inspectable item visual and gives long descriptions internal scrolling. Exact item removal closes the workspace; slot replacement never retargets it. #320 remains open for selection and additional action hardening; #318/#319 remain open for the remaining capability contracts and shared-host completion.
