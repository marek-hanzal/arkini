...
`Argcfile.sh` owns every repository and packaging command; GitHub workflows only install the pinned toolchain and invoke those commands. Working branches run the complete repository gate on hosted Linux, macOS, and Windows, while `main` is the intentional release-only escape hatch. Tagged delivery builds and signs one canonical Arkpack, then macOS arm64, Windows x64, Linux x64, and Linux arm64 jobs embed and byte-compare that same file without rerunning repository checks. Development tags additionally publish those exact bytes as the standalone Arkpack.
