...
- `src/game-runtime` owns the canonical live aggregate: Runtime and committed-transition schemas, runtime validation, live identity creation/revision/removal, aggregate reads and guards, the single mutable store, and atomic mutation publication. `CommittedTransition` remains here because one transaction owns its current and bounded previous Runtime snapshots together with the ordered committed events. Pending item interaction in `src/engine/runtime`, Game Tick, persisted State/save, renderer lifecycle, routes, and Electron may consume Game Runtime but Game Runtime cannot import any of them.
...
- `GameRuntimeLayerFx` builds config plus one `SubscriptionRef<CommittedTransition>` and the `RuntimeFx`/`CommittedTransitionsFx` projections of that exact store. It can create a fresh Runtime or accept an already hydrated Runtime value without knowing persisted State. `GameSessionLayerFx` chooses fresh versus State hydration outside Game Runtime, then composes runtime → Tick → scoped loop. Deterministic tests compose the same runtime and Tick layers from `test/support`; there is no production test façade.
...
- `src/engine` retains framework-neutral session composition, interaction-specific live item/drop operations, runtime query execution, State hydration/projection, save, filesystem/version support, application CLI composition, and other untouched gameplay owners. It no longer owns Game Tick, the canonical live Runtime aggregate, authored Item definitions or selection policy, Game Start, committed Game Event semantics, the production pipeline, spatial locations, placement, merge, or Space actions, and it does not own authored Game source, compilation, Arkpack delivery, or Editor Build.
