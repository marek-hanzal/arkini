...
const ProducerLineOutputQuantityViewSchema = z.union([
...
const ProducerLineEffectRequirementViewSchema = z
	.object({
		kind: z
			.enum([
				"grant.blockStart",
...
const ProducerLineDropEffectViewSchema = z
	.object({
		active: z.boolean(),
		impact: z.enum([
			"availability",
...
const ProducerLineOutputViewSchema = z.object({
	itemId: IdSchema,
	ownedQuantity: z.number().int().nonnegative(),
	enabled: z.boolean().optional(),
	effects: z.array(ProducerLineDropEffectViewSchema).optional(),
	kind: z
		.enum([
			"chance",
			"guaranteed",
	...
	quantity: ProducerLineOutputQuantityViewSchema.optional(),
	rollLabel: z.string().min(1).optional(),
	sort: z.number().optional(),
});

export const ProducerLineViewSchema = z
	.object({
		lineId: IdSchema,
		name: z.string().min(1),
		lineKind: z.enum([
		...
		effectRequirements: z.array(ProducerLineEffectRequirementViewSchema).optional(),
		targetLimits: z.array(ItemTargetLimitViewSchema).optional(),
		outputs: z.array(ProducerLineOutputViewSchema).optional(),
	})
	.superRefine((line, context) => {
		if (line.lineKind === "effect" && line.effectPolarity === undefined) {
			context.addIssue({
				code: "custom",
				message: "Effect producer-line views must expose effectPolarity.",
				path: [
					"effectPolarity",
				],
			});
		...
		if (line.lineKind === "product" && line.effectPolarity !== undefined) {
			context.addIssue({
				code: "custom",
				message: "Producer line views must not expose effectPolarity.",
				path: [
					"effectPolarity",
				],
			});
...
type ProducerLineViewSchema = typeof ProducerLineViewSchema;
export namespace ProducerLineViewSchema {
	export type Type = z.infer<ProducerLineViewSchema>;
}

export type ProducerLineView = ProducerLineViewSchema.Type;
