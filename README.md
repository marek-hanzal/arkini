...
- one exact TanStack Query resource owns the identity of each live route-scoped `Game`; `/game/$packageId` creates or reuses it through `ensureQueryData`, exposes the same instance through inherited router context and loader data, and serializes leave/reset/exit/HMR lifecycle actions through the resource semaphore. Successful disposal removes the query only afterward; failed final saves retain the same frozen retryable game. Native force close terminates the process only after an explicit user decision and does not pretend renderer cleanup or final save succeeded;
