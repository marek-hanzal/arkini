...
export type RuntimeLineOutputView = NonNullable<LineView["outputs"]>[number];
type LineOutputQuantity = NonNullable<RuntimeLineOutputView["quantity"]>;

export interface IndexedLineOutputView extends RuntimeLineOutputView {
...
export const readBonusLinesByItemId = (
...
export const readUniversalBonusLines = (
	entries: readonly RuntimeLineActiveEffectBonusEntry[],
): readonly string[] => {
	const lines: string[] = [];
	for (const entry of entries) {
...
export const collectOutputViews = ({
	bonusLinesByItemId,
	output,
	rollSetLabel,
	save,
...
export const compareOutputViews = (left: IndexedLineOutputView, right: IndexedLineOutputView) =>
	(left.sort ?? maxUnsortedOutputSort) - (right.sort ?? maxUnsortedOutputSort) ||
	left.itemId.localeCompare(right.itemId) ||
	(left.kind ?? "").localeCompare(right.kind ?? "") ||
	left.sourceIndex - right.sourceIndex;

export const isZeroChanceEffectCarrier = (output: RuntimeLineOutputView) =>
	output.kind === "chance" &&
	output.enabled !== false &&
	output.probability === 0 &&
	(output.effects ?? []).some((effect) => effect.impact === "chance");

export const pushOutputSetViews = ({
	bonusLinesByItemId,
	outputSet,
	outputSetIndex,
	rollSetLabel,
