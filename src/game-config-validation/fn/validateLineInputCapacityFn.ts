...
/** Allows positive material buffering capacity only on Common and Clock lines. */
export const validateLineInputCapacityFn = ({
	...
	for (const [itemId, item] of Object.entries(config.items)) {
		if (item.type === ItemTypeSchema.enum.Common || item.type === ItemTypeSchema.enum.Clock) {
			continue;
		}
		...
		for (const { line, path } of lines) {
			for (const [inputIndex, input] of line.input.entries()) {
				...
				diagnostics.push({
					...
					message: `Line ${line.id} owned by ${itemId} cannot buffer materials because only common items and clocks support positive input capacity.`,
					ownerItemId: itemId,
					lineId: line.id,
					inputIndex,
