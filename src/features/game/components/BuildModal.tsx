...
export const BuildModal = memo(function BuildModal({
  ...
  const ownedBlueprintIds = new Set(
    game.inventory.flatMap((slot) => (slot.stack ? [slot.stack.itemId] : [])),
  );
  const availableRecipes = game.buildRecipes.filter((recipe) => ownedBlueprintIds.has(recipe.blueprintItemId));
