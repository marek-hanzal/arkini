...
export namespace useTileStatus {
	export type OperationalState = readTileStatusFx.OperationalState;
...
const unavailable = {
	kind: "unavailable",
} as const satisfies useTileStatus.Projection;

const sameState = (left: useTileStatus.OperationalState, right: useTileStatus.OperationalState) => {
...
const sameProjection = (left: useTileStatus.Projection, right: useTileStatus.Projection) => {
	if (left.kind !== right.kind) return false;
