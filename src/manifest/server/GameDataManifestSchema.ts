  ...
  items: z.array(
    z.object({
      ...
      producer: z.object({
        ...
        drops: z.array(
          z.union([
            z.object({ itemId: ItemIdSchema, weight: PositiveIntegerSchema, quantity: QuantitySchema.optional() }),
            z.object({ itemId: z.null(), weight: PositiveIntegerSchema }),
          ]),
        ).min(1),
        ...
        mode: z.discriminatedUnion("type", [
          ...
          z.object({
            ...
            onDepleted: z.union([z.literal("remove"), z.object({ replaceWithItemId: ItemIdSchema })]),
          }),
        ]).optional(),
      }).optional(),
      build: z.object({
        ...
        resultItemId: ItemIdSchema,
        costs: z.array(z.object({ itemId: ItemIdSchema, quantity: PositiveIntegerSchema })),
      }).optional(),
    }),
  ),
  startingState: z.object({
    inventory: z.array(z.object({ itemId: ItemIdSchema, quantity: PositiveIntegerSchema })),
    board: z.array(z.object({ itemId: ItemIdSchema, x: NonNegativeIntegerSchema, y: NonNegativeIntegerSchema })),
  }),
});
