...
- `sfx/` contains canonical Ogg/Opus sound effects. SFX share the global resource-ID namespace and derive IDs from filenames. `game.json.sfx.events` maps exact SFX event IDs to resources; the vocabulary includes committed gameplay events plus explicit presentation interactions such as Item Detail opening and closing. Each event has at most one assigned sound, and an event without an assignment remains silent. Presentation interactions trigger audio directly without manufacturing a committed gameplay event. Game playback loads and decodes a short effect only when its assigned event occurs; build validates and streams every SFX source unchanged.
