...
Notes are stored once in `notes/<noteId>.json` with Markdown content, ordering/freshness timestamps, and an optional unique `itemUids` array. A missing or empty link array means a global note and is normalized to an empty array when read; each link uses the immutable item UID, never its renameable authored ID. The global Notes route and every Item detail Notes tab share one composer/list; item forms do not expose Notes. Create/edit validates links against the open project and guards updates by `expectedUpdatedAtMs`. Item/config deletion strips every absent UID, including force-deleted owners, in the same recoverable current-tree transaction, preserving the note and advancing its freshness.
