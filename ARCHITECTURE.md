...
`Planner` is the public orchestration capability above those strategies. One planner instance owns the immutable acquisition graph and authored start runtime, registers the available strategies, resolves a deterministic fallback sequence, retains each attempt for diagnostics, and turns the winning engine-valid execution trace into expected economics. The production-compatible default remains best-first until constructive coverage is proven; callers may explicitly request a constructive-first fallback for A/B work. Strategy concurrency or portfolio execution may later change scheduling, but not the engine boundary or the normalized planner result.

`EnginePlanner` temporarily preserves the historical `searchFx` and `constructiveSearchFx` entry points for editor and audit callers. Those methods delegate to the registered strategies and are not separate planner implementations. New orchestration enters through `Planner.estimateFx`; compatibility methods may disappear only after their consumers migrate to strategy-neutral results.
