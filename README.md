...
The client uses TanStack Router file-based routing. `/` owns a one-renderer-session startup splash and authoritative launcher bootstrap, `/main-menu` owns the semantic out-of-game menu, `/arkpacks` owns the existing local package selector, `/about` owns credits, and each exact package opens under `/game/$packageId`. Official Arkini and validated imported packages share one root-owned catalog backed by Electron storage; TanStack Query is used only for concrete asynchronous UI commands and never owns catalog or gameplay truth. One root-shell `GameOwner` binds the active game route declaratively to an explicit serialized lifecycle: package selection, route release, controlled shutdown, hard reset, and invalid-save recovery run under one Effect semaphore with no command queue or latest-intent scheduler. The selected game renders the current board through a headless tile system. Inventory and gameplay commands remain future slices.
...
Appearance is renderer-owned and exposed through semantic Tailwind color utilities backed by one CSS token palette. The user may explicitly select `dark`, `light`, or `system` plus one of the supported semantic accent palettes. Missing or malformed preferences resolve to dark and rose. Electron persists both selections atomically under `userData/arkini/preferences`, applies the theme through `nativeTheme`, and exposes no browser-storage settings path.
...
- package selection, route release, HMR handoff, controlled close, hard reset, and invalid-save recovery pass through explicit root `GameOwner` operations under one semaphore. Commands fail honestly and publish matching UI state; no load mutates a running session in place. Failed final saves retain the same frozen retryable game. Native force close terminates the process only after an explicit user decision and does not pretend renderer cleanup or final save succeeded;
- package validation failures never expose save deletion because no save identity is trusted. When a verified package fails specifically during durable save decode or runtime hydration, the root owner retains the exact `packageId + contentHash` key privately and exposes only a recovery capability to UI. Recovery clears only that exact save and reruns the normal bootstrap; a failed post-clear bootstrap retries without clearing the same key twice;
