...
export const readNavigationFx = Effect.fn("readNavigationFx")(
	(
		...
		Effect.sync((): ReadonlyArray<string> => {
			if (!flow.nodes.some(({ id }) => id === startNodeId) || !positions.has(startNodeId))
				return [];
			...
			const visit = (nodeId: string, previousNodeId?: string) => {
				...
				ordered.push(nodeId);
				const previous =
					previousNodeId === undefined ? undefined : positions.get(previousNodeId);
				const targets = [
					...
					const turnDifference =
						readTurnCost(previous, current, left) -
						readTurnCost(previous, current, right);
					...
					const distanceDifference =
						readDistance(current, left) - readDistance(current, right);
