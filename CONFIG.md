...
- `limit` is a shared rule condition shaped as `{ "type": "limit", "itemId": "target" }`, available wherever conditions are authored, including line and drop rules. It has no query scope or numeric range. The Editor offers only items with `maxCount` and clears an uncapped selection when switching to Limit. Runtime compares all existing quantities with the selected item's current `maxCount`; future output reservations are excluded and removing the cap makes the condition false;
...
- Item `ui` is `simple` or `default` (default `default`). Simple shows only item information, hiding the entire section navigation; Default always exposes all Item Detail tabs even with no production lines. Both interfaces allow Board clicks to enqueue the effective Default line, including queue fill. Without a Default line, no click production is available. Simple still denies direct line commands, input management, other queue changes and line selection; Default permits ordinary manual production. Clock, Action, movement and merge policies remain independent;
