...
import { registerSerakkiElectronIpcFx } from "~electron/main/registerSerakkiElectronIpcFx";
...
export const createRegisteredIpcHarness = async () => {
	...
	const userDataPaths = createSerakkiUserDataPathsFn(userDataPath);
	const assertTrustedIpcSenderFx = vi.fn((event: IpcMainInvokeEvent) =>
		event.senderFrame?.url.startsWith("serakki://app/")
			? Effect.void
			: Effect.fail(
	...
	await Effect.runPromise(
		Effect.gen(function* () {
			...
			yield* registerSerakkiElectronIpcFx({
				bundledSerapacksRoot: join(userDataPath, "bundled-serapacks"),
