...
- 2026-07-09 runtime-bridge follow-up after `34e7a466`: `readRuntimeLootDropViewsFromEffectiveLine.ts` now consumes canonical loot-plan view entries from `effects` instead of rebuilding weighted/chance traversal locally. `readRuntimeLineOutputViews.ts` was simplified to trust canonical entry ordering, and two single-use bridge wrappers (`readRuntimeLineJobs.ts`, `readRuntimeLineStartRequirementsReady.ts`) were inlined into `readRuntimeLineViewFromDefinition.ts`. Remaining bridge ROI is now increasingly concentrated in larger projection builders rather than helper clutter.
