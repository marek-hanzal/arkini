...
> Continue #317 incrementally from the landed first #320 hover-action slice. The next discussion/implementation step should replace the temporary all-tile action set with authoritative capability availability and then connect one explicit action to the shared workspace host; do not add selection, shortcuts, or capability content ahead of that agreed slice. Preserve the existing TileActor/DnD owner, Floating UI as positioning support only, Arkpack-owned capability icon resources, and the no-second-geometry/no-second-gameplay-truth contract.
