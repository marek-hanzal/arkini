...
Arkini is the presentation source of truth. It owns actor phase, operation generation, source/target identities, topmost `slot | surface | outside` hit testing, z-order, engine handoff, and live reconciliation targets. Motion listens to that state and owns physical drag tracking, frame batching, transforms, scale, opacity, velocity continuity, and spring interpolation. Do not derive gameplay or presentation truth back from Motion. Pointer-frequency Motion updates must not drive a React render loop. Use an outer Motion-owned anchor shell (`left/top`) and an inner Motion-owned drag/settle shell (`x/y`) so placement and gesture transforms never overwrite each other.

Every release invokes one public atomic engine drop command. UI must never infer merge, consume, swap, stacking, storage eligibility, placement fallback, or removal semantics. The command returns explicit actor identities and previous/current locations for move, swap, merge, ignored, and reject presentation. A merge source first approaches the live target; its surviving stack identity may then return to its current anchor, while consumed/removed identities exit and a surviving or replaced target receives one restrained impact. The actor layer may retain the last renderable snapshot of a removed identity only while the current operation generation still owns that exit. Rejected or unsupported targets are truthful outcomes and return toward the newest canonical live anchor. Hover is a pronounced preview state around `1.15×`; drag must remain at least as prominent. Scale, active shadow/lift, settling, rejection, impact, opacity, and exit are Motion-owned and must not compete with CSS transitions or keyframes.
