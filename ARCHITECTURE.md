...
The editor project context contains only the last successfully persisted and compiled source snapshot. Item routes use immutable item UID leaves: `items/list`, `items/$itemUid/view`, and explicit `items/$itemUid/create/<section>` or `items/$itemUid/edit/<section>` form leaves. Create and edit parent routes each own exactly one local TanStack Form session above their section outlet, so Identity, Artwork, Limits, Charges, Merges, and type-owned Production routes share values, dirty state, touched fields, and validation without a global draft. View is canonical and read-only; leaving the create/edit parent route discards its local form. There is no global staged-item registry, project overlay, or draft recovery layer. The form keeps presentation values such as comma-separated tags and temporarily blank required numbers in its own store, runs the canonical item schema through submit-first/change-after-failure validation, and publishes field issues at their exact paths. A complete-item validation failure navigates to the section owning the first actionable field. Only a successfully parsed complete item reaches the save command. Save then upserts it against the current source compilation by UID, compiles the complete candidate, writes the owning source file under revision CAS, and only then publishes the resulting canonical project snapshot. Human-readable item IDs remain editable authoring keys and never determine object identity or create-versus-edit intent.
...
Electron exposes one canonical project writer rather than a generic file writer. The first project load reads the complete contained JSON/PNG workspace once and seeds matching main-process and renderer in-memory file indexes. Subsequent editor navigation reuses the canonical renderer snapshot. A successful save compiles against that index, commits only the changed content file together with a monotonically newer `editor.json.updatedAtMs`, advances the cached revision, and returns only the exact persisted delta. Neither side rescans the project or retransports unchanged PNG bytes after ordinary user interactions. Renderer code cannot write `editor.json` directly. Recent-project ordering, persisted content, and the canonical renderer snapshot therefore advance through the same writer.
