...
export const GameLoadingScreen = ({ onComplete, ready }: GameLoadingScreen.Props) => {
	...
	return (
		<section
			className="relative flex size-full min-h-0 min-w-0 flex-col items-center justify-center gap-[clamp(1.25rem,4vmin,2.5rem)] overflow-hidden bg-canvas p-[clamp(1rem,4vmin,3rem)] text-foreground"
			data-ui="GameLoadingScreen"
		>
			<div
				className="h-[35dvh] min-h-28 w-[min(80dvw,56rem)]"
			...
			<div
				className="h-2 w-[min(64dvw,28rem)] overflow-hidden rounded-full border border-line bg-surface-raised/60 shadow-inner"
