...
describe("dropItemFx pure stack integration", () => {
	...
	it("serializes competing stack commits without overflowing or losing quantity", async () => {
		...
		expect(
			result.attempts.filter(({ kind }) => kind === DropItemResultKind.Stack),
		).toHaveLength(1);
		expect(result.attempts).toContainEqual({
			kind: DropItemResultKind.Reject,
			reason: DropItemRejectedReason.StaleTarget,
			itemId: expect.stringMatching(/^runtime:(?:first|second)-source$/),
			targetItemId: "runtime:target",
		});
