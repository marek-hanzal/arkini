...
- `src/renderer` contains only concrete renderer-process runtime, lifecycle, concurrency, and remaining transport capabilities. Save transport belongs to Game Persistence. Renderer is not a required gateway to Engine, Editor, or `electron/contract`; callers import the exact owner directly.
