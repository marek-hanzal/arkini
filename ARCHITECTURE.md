...
The editor project context contains only the last successfully persisted and compiled source snapshot. Item routes use immutable item UID leaves: `items/list`, `items/$itemUid/view`, `items/$itemUid/edit`, and `items/$itemUid/create`. View is canonical and read-only; edit and create own one local TanStack Form value that disappears when its route unmounts. There is no global staged-item registry, project overlay, or draft recovery layer. A form submit parses one complete item, upserts it against the current source compilation by UID, compiles the complete candidate, writes the owning source file under revision CAS, and only then publishes the resulting canonical project snapshot. Human-readable item IDs remain editable authoring keys and never determine object identity or create-versus-edit intent.
