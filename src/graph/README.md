...
{"kind":"operations","operationKinds":["merge"],"filter":{"action":"spend"}}
{"kind":"operations","operationKinds":["line"],"search":{"text":"Digest","scope":"title"},"filter":{"clock":true,"show":false,"enable":true,"clockWeight":{"gt":15}}}
{"kind":"operations","operationKinds":["merge"],"participant":"item:puppy","role":"target"}
{"kind":"path","from":"item:A","to":"item:B","direction":"out"}
```

MCP defaults/maxima: depth 1/12, result limit 50/200, expansions 10,000/100,000 and cooperative timeout 1,000/5,000 ms. Chain defaults to depth 5. Internal Editor queries retain their separate 3/12 depth and 100/1000 result defaults/maxima. Limits count unique returned edges for connection/traversal, paths for path queries, and operations for node lookup or operation listing. Path responses can include up to depth × path-limit edges. Depth-limited frontiers remain partial. Ordering is deterministic by canonical identity except for operation search, which uses stable relevance order; paths use breadth-first traversal and stable branch order.

Operation pages return a compact opaque `nextCursor` when a scan can continue. The session retains at most 1,024 issued continuation tokens in FIFO order; expired tokens require rediscovery. Repeat the operation query with unchanged filters, its cursor, revision and snapshotId; a cursor cannot silently continue against different content or a different filter. A bounded page that has not found a match is incomplete, not evidence that no operation exists.

`graph_query_batch` takes 1–8 uniquely named `{id, query}` entries. Each `query` must be an object; its fields are validated independently so malformed query fields remain per-entry errors. All subqueries run against one captured immutable snapshot. Internally, node, edge and operation collections are deduplicated by identity and each query keeps references to its own records. MCP presents common project/revision/snapshot metadata once, then a readable section per query ID with its own status, truncation, reasons, paths, ordered operations, match evidence and continuation; it does not expose the storage tables. A failed subquery reports its own error without replacing successful siblings. Batch revision/snapshot admission fails before any subquery when the requested snapshot is stale.
