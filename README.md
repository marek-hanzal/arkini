...
MCP `graph_schema_json` documents the focused graph tools and selective hydration. `graph_search` finds titled nodes by name or identity; `graph_connections` reads direct relationships; `graph_operations` audits authored operations with participant, search and scalar filters. `graph_path` finds structural paths, while `graph_flow` follows atomic authored transformations with explicit prerequisites. `graph_traverse` explores broader structural neighborhoods and requires explicit edge kinds beyond one hop. `graph_batch` runs up to eight named queries on one snapshot, with independent statuses and presentation. Discovery returns compact text with short operation references, exact node IDs and snapshot pins. Connections and operations support compact continuation. `graph_operations_json` hydrates selected operation references; `items_json` and `item_lines_json` provide complete item/line documents. See the [graph contract](src/graph/README.md).
