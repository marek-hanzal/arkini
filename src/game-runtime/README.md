...
### Item-owned generated spaces

[`space/resolveGeneratedSpaceFx`](../space/fx/resolveGeneratedSpaceFx.ts) owns lazy allocation and initialization in the caller's unpublished draft. Runtime Item and State Item carry the single optional `generatedSpace` binding; existing `templateUidBySpace` owns active geometry. There is no separate ownership registry. Allocation reserves every authored literal and occupied current/snapshot address. Outcome application passes the exact definition owner; receiver transport passes its target. A dead owner cannot create a room; repeated use returns the bound address. Same-ID replacement explicitly preserves the binding.

[`destroyGeneratedSpacesFx`](../space/fx/destroyGeneratedSpacesFx.ts) integrates with permanent identity removal, passive subtree discard and template replacement. It destroys room contents, complete input/job trees, deliveries originating there and nested owned rooms, without lifecycle settlement or refunds. Detached-job callers pass their pre-settlement ancestry explicitly. Captured input, unit, job and outcome work stops when its identity or room was destroyed. Destruction removes active-template metadata, clears invalid history, and falls back from a deleted current room to surviving history or configured start. [`isSpaceRetainedFn`](../space/fn/isSpaceRetainedFn.ts) compares owner identity across snapshots so recycled numbers cannot retain old navigation references.

[`checkGeneratedSpacesFn`](../space/fn/checkGeneratedSpacesFn.ts) checks unique bindings, separation from authored addresses and existing template initialization at commit and hydration. Shape changes affect persistence, destination authoring, template-reference validation, graph recipe projections and generated project schema. Ordinary placement geometry, global switches and unrelated fixed spaces retain their existing owners.
