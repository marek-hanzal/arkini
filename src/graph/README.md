...
MCP defaults/maxima: depth 1/12 for traverse and 5/12 for path/flow, flow result limit 5/200 and other discovery result limits 50/200 (also inside batches), expansions 10,000/100,000 and cooperative timeout 1,000/5,000 ms. Editor Chain defaults to depth 5. Internal Editor queries retain their separate 3/12 depth and 100/1000 result defaults/maxima. Limits count unique returned edges for connection/traversal, paths for path queries, operations for operation listing, groups for grouping (count ignores result limit), nodes for search, and flows for operation-path search. Flow depth counts operations; structural depth counts relationships. Path responses can include up to depth × path-limit edges. Depth-limited frontiers remain partial. Ordering is deterministic by canonical identity except for node and operation search, which use stable relevance order; paths use breadth-first traversal and stable branch order.
