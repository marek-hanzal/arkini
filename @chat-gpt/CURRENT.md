...
**UI state-style ownership cleanup**

Status: **Issue #354 is implemented on `main`. `ak-list` now owns one plain non-zebra content-row surface only. Canonical Button variants no longer receive list-row classes in MainMenu, GameMenu, or the Cheats back action, so their full base/hover/focus/active/disabled contrast remains variant-owned. Settings theme choices use a local segmented-radio grammar, while genuine Settings/Cheats content rows retain the restrained list-row interaction grammar. Cheat Spotlight options now use their own complete neutral/selected/focus/hover token and class contract, including selected title, configured ID, and category contrast. Item Detail Lines/Sources and Arkpack catalog remain genuine list consumers; tile grids remain excluded.**

Recent Cheat baseline remains a strict application/save split. `/settings` persists one application-wide `cheatsAvailable` preference that only exposes save-scoped cheat tooling and never reads or mutates a Game. `/game/$packageId/cheats` owns the exact save state `{ enabled, everEnabled, instantGameplay }`; each save must explicitly enable Cheat mode, and the irreversible `everEnabled` marker remains true after the first enable for future reward/progression policy. The GameMenu destination and direct route require application availability, while save behavior and engine commands remain authorized by that save's current `enabled` value. Effective Instant gameplay is `enabled && instantGameplay` and removes waiting only through bounded canonical fixed-step settlement, preserving command validity, inputs, charges, RNG, placement, lifecycle, rollback, and passive-storage pause semantics. The Canvas-local item Spotlight uses TanStack Hotkeys `Mod+P`, additionally respects application availability, reads an engine-owned compiled catalog, and spawns through one authorized canonical Board placement command. Legacy speed, Nuke, and Cheat-inventory items, runtime session speed, schema/event/UI branches, config entries, tests, and unreferenced assets are removed; the normal Inventory Backpack and GameMenu reset remain intact.
...
Item Detail now also exposes an authoritative optional `sources` tab. It lists only exact currently owned line owners placed on Board, across every space, whose canonical Lines projection visibly produces the inspected item; passive-storage owners, hidden lines, theoretical recipes, merge/reward/lifecycle outputs, and temporary startability filtering are excluded. Current-space sources sort first, then spaces/title/exact identity deterministically. Every source row owns one legitimate runtime identity and hands off through the stable modal to that exact owner's `lines` tab; stale inspected targets retain the last source list read-only, while live source removal removes the capability and falls back through the normal tab resolver.
...
> Run an independent review pass over the completed #298 Cheat runtime snapshot before beginning another large feature. Verify the application/save boundary, irreversible per-save `everEnabled` marker, game-scoped route lifecycle, bounded Instant settlement, Spotlight authorization/placement, and complete legacy-slice removal without reopening the deleted item-based architecture.
...
- `/` is the one-session startup splash, `/main-menu` is the semantic out-of-game menu, `/arkpacks` contains the shared package selector, `/settings` owns application appearance plus application-wide cheat-tool availability, `/about` contains credits, `/game/$packageId` is the live resource boundary, `/game/$packageId/board` is gameplay, and `/game/$packageId/cheats` is the exact save-scoped cheat page. Hash routes, implicit game index pages, and `file://` are not supported application modes.
...
- Application-wide cheat-tool availability is one Electron preference exposed through a renderer-wide live owner. `/settings` can read and persist it without an active Game and never reads, mutates, or mirrors save cheat state. Enabling availability changes no existing or future save automatically.
- `runtime.cheats` is canonical persisted state for the exact loaded Game: `{ enabled, everEnabled, instantGameplay }`. `StateSchema`, save/load, runtime publication, reset, and Game replacement carry one value set; route state, React, Query, and runtime session contain no duplicate save truth.
- `everEnabled` is an irreversible save taint marker. `setCheatEnabledFx(true)` sets it permanently, while later disabling Cheat mode preserves both the marker and stored options so future reward/progression policy can distinguish that save.
- `/game/$packageId/cheats` is a named visual child of the exact Game route. Application availability exposes its GameMenu entry and authorizes direct route access; the page then owns the explicit per-save Cheat mode toggle plus save options. Native browser history returns to the same Board instance, and removing application availability exits to that package Board without mutating save values.
...
- `Mod+P` is owned by TanStack Hotkeys on the active gameplay Canvas. The Spotlight opens only while application tooling is available and the exact save has Cheat mode enabled, reads an immutable engine-owned compiled catalog, and invokes one public Cheat spawn command that reauthorizes save `cheats.enabled` and uses normal Board creation, max-count, stacking, placement, publication, and save behavior.
