...
- Durable item storage is normalized through one `itemInstance` table. Board items and inventory stacks are projections over the same saved item entity, with `locationKind` deciding whether the instance is active on the board, passive in inventory, or reserved for future input-storage locations. The old physical split between `boardItem` and `inventoryStack` is gone from runtime code because making one item pretend to be two database species was cute only in the same way a car fire is warm.
