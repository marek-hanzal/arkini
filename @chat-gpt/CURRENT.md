...
- The first #320 hover-action slice is implemented without modal or selection ownership. Every live Board/Toolbar actor owns one conditionally mounted Floating UI action strip anchored to its transformed drag surface, preferring `bottom` with offset/flip/shift collision handling and animation-frame auto-update. The temporary stable actions are Info, Status, Lines, and Effects rendered as emoji placeholders; they intentionally execute nothing yet. Hover uses a safe pointer corridor, and all actor strips are suppressed during `pressed`, `dragging`, and `awaiting-outcome`; post-outcome `settling` remains hover-capable under the immediate-reinteraction contract. Retained/exiting non-live actors never expose actions. #320 remains open for authoritative capability filtering, modal handoff, and selection.
...
> Continue #317 incrementally from the landed first #320 hover-action slice. The next discussion/implementation step should replace the temporary all-tile emoji actions with authoritative capability availability and then connect one explicit action to the shared workspace host; do not add selection, shortcuts, or capability content ahead of that agreed slice. Preserve the existing TileActor/DnD owner, Floating UI as positioning/hover-path support only, and the no-second-geometry/no-second-gameplay-truth contract.
