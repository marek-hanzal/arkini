 ...
 */
export const readPlannerStructuralReachability = ({
	graph,
	itemId,
}: {
	...
	for (let index = 0; index < pending.length; index += 1) {
		...
		if (candidateItemId === undefined || unreachableItemIds.has(candidateItemId)) continue;
		...
		for (const route of routes) {
			...
			pending.push(...blocked.missingAllOfItemIds, ...blocked.missingAnyOfItemIds.flat());
		}
	}
