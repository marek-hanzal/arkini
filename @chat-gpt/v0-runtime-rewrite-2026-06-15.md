...
- drag-facing commands patch React Query optimistically before the DB round-trip (`board.move`, `board.swap`, `board.merge`, `inventory.swap`, `inventory.place`, `inventory.stash`)
...
- drag commit is delayed until after the snap animation so optimistic cache writes do not teleport the actor mid-drag
