...
- the integrated editor stores canonical projects in a main-process SQLite database under `<userData>/arkini/editor/projects.sqlite`; the renderer uses a narrow validated IPC repository and never owns persistence. Editor database failure is isolated from gameplay startup: Play and global Settings remain available while Editor is disabled. Local form values are the only draft; an explicit form or asset Save commits one atomic project mutation. Arkpack import validates the compressed package before creating a project. JSON import selects a source directory read-only, runs the canonical completed-game compiler and resource validation, reads its PNG resources, and creates the same canonical project shape. Both imports use one transactional project/resource insert and reject duplicate project identities. Build creates immutable Arkpack bytes that may independently be saved or installed into the catalog. Its separate JSON source export captures the current saved revision and writes `schema.json`, `game.json`, item-type directories, and every PNG resource directly into the selected, re-importable root. That root is explicitly Editor-managed: after native destructive confirmation, a complete staged output atomically replaces its entire existing file tree. Symlink-resolved guards reject overlap with source, application, or Arkini data trees, and controlled close joins any admitted export. Imported source workspaces are never mutated implicitly and the signed application bundle is never written. Existing renderer-origin IndexedDB data is deliberately not migrated by this storage cutover.
