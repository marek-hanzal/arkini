...
- Choose The Path is now represented by three Era XI keystone building items: `producer:house-of-engineers`, `producer:cathedral`, and `producer:mage-lodge`. University issues their blueprints after prestige/treasure inputs. The buildings are mutually exclusive through `exclusiveToIds` and currently act as branch marker items for later `showIf` product variants.
