...
- Space outcome and receiver transport destinations use `space: <nonnegative integer> | "previous"`. Previous Space means the last space left by committed player navigation, resolved at settlement. Without history the outcome is skipped and receiver transport is rejected atomically. Editor destination controls explain this only in their info tooltip; authored graphs preserve `space:previous` as a dynamic reference without inventing a concrete destination.
