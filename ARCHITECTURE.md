...
- The renderer entry declares `<base href="/">`, so generated relative assets resolve from the `arkini://app/` origin root even when the current history route is nested. Every production build verifies the generated `.out/desktop/build/renderer/index.html` asset graph from root, game, and development routes.
...
Electron `userData` has one Arkini root with explicit game and editor namespaces:
...
The editor is another renderer surface around the same engine/compiler contracts, not a parallel application. Import validates the compressed Arkpack through the normal trust, payload, semantic, and resource boundaries, then projects the completed config into deterministic source fragments. Game-wide fields remain in `game.json`; item domains own their editor source projection and future forms and are grouped by item type because an Arkpack does not preserve authoring-era provenance; shell resources remain separate from item assets. Electron owns only contained, ordered filesystem publication below the editor root and exposes typed create/read/open-directory capabilities. Renderer UI never receives arbitrary filesystem authority.

Editor bridge code follows the same domain ownership as the rest of the renderer: project session, workspace, and mutation-lane orchestration remain under `bridge/editor`; item form projection and item persistence live under `bridge/item/editor`; asset persistence lives under `bridge/resource/editor`; Arkpack-to-workspace import lives under `bridge/arkpack/editor`. The editor root is not a catch-all namespace for every capability used by an editor screen.
Reusable editor lifecycle, revision, validation, and persistence operations are same-named `*Fx` programs. Whole-item create seeds are bridge-owned React hooks, while nested local form seeds stay presentation-owned constants or inline callbacks; neither introduces a second exported synchronous operation grammar.

The editor provider owns the last successfully persisted project plus a session-local overlay of validated staged items. Item routes use immutable item UID leaves: `items/list`, `items/$itemUid/view`, and one `items/$itemUid/form/$sectionId` flow for both new and persisted items. The form parent owns exactly one local TanStack Form session above its section outlet, so Identity, Artwork, Limits, Charges, Merges, and type-owned Production leaves share values, dirty state, touched fields, and validation. The form keeps presentation values such as comma-separated tags and temporarily blank required numbers in its own store, runs the canonical item schema through submit-first/change-after-failure validation, and publishes field issues at their exact paths. A complete-item validation failure navigates to the section owning the first actionable field. Form Save validates the complete item and stages it in the provider-owned project draft without touching the filesystem. The shell's project Save persists the captured staged items serially through the canonical mutation lane; each successful prefix item is removed from the draft and a failed item plus its tail remain available for retry. Human-readable item IDs never determine object identity or create-versus-edit intent, remain editable until the item's first successful project persistence, and are read-only afterward pending an explicit rename workflow.

Ordinary editor navigation discards an unsubmitted local form without interception; already staged items survive child-route navigation for the editor project session. The mounted form publishes only its command surface and dirty status to the shell, never its editable values. A fixed-height status slot below the editor navigation exposes form-level Discard and Save actions without shifting content. The shell's top Save is the separate project-persistence command and is disabled while the active form has newer unstaged changes. Exit is the sole guarded navigation: clean state closes immediately, while local or staged changes require Save or Discard before the project session is released.
...
Electron exposes one canonical project writer rather than a generic file writer. The first project load reads the complete contained JSON/PNG workspace once and seeds matching main-process and renderer in-memory file indexes. Subsequent editor navigation reuses the canonical renderer snapshot. A successful save compiles against that index, stages the changed content and manifest, publishes content first and the monotonically newer `editor.json.updatedAtMs` commit marker last, advances the cached revision, and returns only the exact persisted delta. Runtime publication failures attempt rollback and retain a recoverable backup if restoration itself fails; this is deliberately not a cross-file crash-atomic transaction. Neither side rescans the project or retransports unchanged PNG bytes after ordinary user interactions. Renderer code cannot write `editor.json` directly. Recent-project ordering, persisted content, and the canonical renderer snapshot therefore advance through the same writer.
