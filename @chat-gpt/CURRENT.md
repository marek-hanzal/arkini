...
Status: **Epic #343 is implemented on `main` as the authoritative replacement for the old Floating UI hover/workspace experiment. Exact live Board, Toolbar, and Inventory actors open one Canvas-local Item Detail modal through desktop double-click or the shared programmatic entry. The modal owns one exhaustive generation-safe lifecycle and finite `info | status | lines | queue` tabs without routes, URL state, duplicate shells, or React gameplay mirrors. The old hover toolbar, hovered hotkeys, capability icons/config/resources, item Effects inspector, and direct bridge `Effect.runSync` islands are removed. Status and Lines now share canonical engine-owned `idle` versus `ready` truth. Lines expose explicit Start/Enqueue, Autofill, Withdraw, active-work status, and Queue is a separate focused tab with canonical Clear queue semantics. UI test files use deterministic isolation rather than leaking hoisted singleton mocks.**
...
- Item Detail contains one shared header, one stable tab row, and one internally scrolling body. `info` is always available. `status`, `lines`, and `queue` are exposed only by the authoritative engine capability result. Unsupported requested tabs fall back deterministically to `info`; tab switching preserves the same modal shell and target.
- Status uses one engine-owned owner grammar: `idle` means no active work and no line can start now; `ready` means no active work and at least one line can start now; `working`, `stored`, and `paused` retain their canonical meanings. Status and Lines reuse the same line-start resolution. React never reconstructs readiness from location, inputs, output space, charges, or jobs.
