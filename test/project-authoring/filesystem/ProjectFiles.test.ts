...
describe("filesystem Editor project current tree", () => {
	it("round-trips and republishes the complete authoritative current tree", async () => {
		...
		const waterPath = join(harness.root, "items", "common", "water.json");
		...
		await expect(
			access(join(harness.root, "items", "common", "water.json")),
		).rejects.toBeDefined();
		expect(
			JSON.parse(
				await readFile(
					join(harness.root, "items", "common", "water%2E%ED%A0%80.json"),
					"utf8",
				),
			),
		).toEqual({
