...
Tile interaction uses one Canvas-wide Motion actor system shared by the rendered Board, Inventory, and Toolbar. The Inventory is a non-modal Canvas-local surface and the optional Toolbar is one passive row; all three expose stable slot anchors only. `TileActorLayer` renders exactly one visible stable DOM actor for each live runtime item identity, independent of its current storage surface, so Board ↔ Inventory ↔ Toolbar movement never creates a clone. Arkini owns actor phase, operation generation, source/target semantics, z-order, topmost `slot | surface | outside` hit testing, engine command selection, and reconciliation targets. Motion receives that Arkini-authored state and owns the physical drag gesture, frame batching, scale, opacity, velocity continuity, and spring interpolation. Pointer press is visually inert: it only arms gesture arbitration and preserves the current stable or hover pose. Once Motion crosses the drag threshold, the actor uses a Motion-owned `left/top` anchor shell, an exact `x/y` pointer drag shell, and a nested short pickup-correction shell that eases an off-center grab toward the actor center without filtering pointer movement. Settlement folds both local offsets into the current visual pose before resetting them, so pointer tracking and placement transforms never compete or snap. The centered inner tile body rests at scale `0.8`; hover and free drag use the restrained `0.9` pose, while accepted, ignored, rejected, targeted, impact, and exit states publish their own explicit visual targets. Every release invokes one public atomic engine drop command; the UI never infers move, merge, consume, swap, stacking, or storage eligibility. The command resolves empty-slot move, non-mergeable occupied swap, compatible stack transfer, authored directional consume merge, same-slot ignore, and explicit reject across the three surfaces. Merge outcomes expose exact previous/current source and target identities; source approach is followed by independently completing return, impact, or exit phases. Removed gameplay identities may remain in the actor layer only while the active presentation generation protects them, and synchronous engine publication cannot create a one-frame unmount between command commit and outcome settlement. No ghost, clone, screenshot, hidden canonical tile, per-surface actor tree, manual RAF drag runtime, direct WAAPI path exists.
...
Controlled Electron close is not a route transition. Preload joins the pending or published singleton resource, performs one best-effort `disposeFx` attempt, logs a failed final save, and still sends `closeReady`; the renderer process teardown is the terminal cleanup fallback. No retry page, second save loop, or fatal-screen detour participates. HMR remains stricter: it uses the normal release operation, and a newly created resource waits for the previous HMR shutdown promise so sessions cannot overlap across module replacement. Explicit native force close remains process policy and never claims renderer cleanup or save success.
