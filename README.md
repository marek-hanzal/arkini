...
Disposable application output lives below `.out/`: Electron build files under `.out/desktop/build`, distributable artifacts under `.out/desktop/release`, and tool caches under `.out/cache`. Each authored game owns its ignored canonical artifact below `<project>/build/`; the tagged development-prerelease packaging additionally creates its Sigstore bundle there.
...
Arkpack and game-save persistence are Effect-native inside the renderer bridge and Electron main. Promise is used only by the typed preload/IPC transport. Main filesystem behavior is composed from narrow object factories over the `FileSystem` and `Path` capabilities exported by `effect`; `@effect/platform-node` supplies their one Electron/CLI implementation. The Node-only `FilesystemWrite` capability supplies portable optimistic cross-process locks and recoverable file replacement without becoming a domain repository. There are no project-owned repository/storage classes or no-op close contracts.
...
- package binaries and gameplay saves are separate storage boundaries. Removing an imported package does not delete its save; a package with the same gameplay major may resume it regardless of minor ordering, while a different major rejects restore and preserves the save;
...
- the engine owns the portable game-project filesystem format: generated root `schema.json`, `project.json`, `game.json`, `items/<type>/<uid>.json`, `assets/`, `resources/`, `notes/`, `scenarios/`, `versions/`, and `objects/`. The Editor and `arkini-cli game pack` consume the same format. `project.json` contains only the Arkini writer version and project `revision`; `game.json` contains the authoritative package ID in `meta.id`, gameplay `version`, and a `./schema.json` reference, while each UID path contains one direct `item` referencing `../../schema.json`. The marker, schema, and references must match the exact current contract. Electron main owns only the `<userData>/arkini/editor/projects.json` root catalog and the narrow validated IPC repository; project identity is always read from the validated root's `game.json` `meta.id`, never copied into the catalog. Startup always reconciles direct managed directories into that index without recursively deleting an unlisted root; valid external bindings survive only a valid existing catalog. New projects and validated Arkpack imports create managed directories below user data, while project-folder import requires the root marker and opens an external directory directly in place. Project operations share canonical `editor.lock` exclusion and recover interrupted exact-file writes before reading or mutating the tree. Each mutation writes and deletes only its Arkini-owned paths, preserving `.git` and unrelated files in external roots; version checkout updates the current tree, scenarios, and head together. Existing managed projects are removed only by the explicit delete command. There are no migrations or compatibility readers. Explicit Refresh still hard-resets drafts and the Editor Board. JSON export creates a new owned child inside the selected destination, copies only the portable allowlist, and never replaces an existing path. Version manifests reference full content-addressed objects and become visible only through `versions/head.json`. Installation-wide MCP configuration and OAuth state share `editor/mcp.json`.
- the Editor Board runs the same canonical session commands and routed Board, Toolbar, Inventory, Item Detail, audio, cheat spotlight, and Pixi UI as installed gameplay. Its process-owned lifecycle has no autosave and serially discards the old session before publishing a changed project revision, restoring a scenario, or refreshing from disk. `Cmd+P` is always admitted there through canonical Cheat mode. The live Board is excluded from portable project state. Named scenarios are portable, versioned JSON envelopes around `StateSchema` bytes: every same-major gameplay version restores regardless of minor ordering, failed reads preserve the scenario, and every major project save removes all scenarios before publishing the new project marker while minor saves preserve them.
