...
export async function placeInventoryItem(slotIndex: number, x: number, y: number) {
	const input = PlaceInventoryItemInputSchema.parse({
		slotIndex,
		x,
		y,
	});
	await db.transaction().execute(async (tx) => {
		...
		if (boardRows.some((row) => row.x === input.x && row.y === input.y))
			throw new GameActionError("Board cell is occupied.");
...
export async function swapInventorySlots(sourceSlotIndex: number, targetSlotIndex: number) {
	const input = SwapInventorySlotsInputSchema.parse({
		sourceSlotIndex,
		targetSlotIndex,
	});
...
export async function stashBoardItem(boardItemId: string, slotIndex?: number) {
	const input = StashBoardItemInputSchema.parse({
		boardItemId,
		slotIndex,
	});
	await db.transaction().execute(async (tx) => {
...
export async function moveBoardItem(boardItemId: string, x: number, y: number) {
	const input = MoveBoardItemInputSchema.parse({
		boardItemId,
		x,
		y,
	});
	await db.transaction().execute(async (tx) => {
		...
		if (occupied)
			throw new GameActionError("Drop on an empty board cell or merge a valid recipe.");
...
export async function mergeBoardItems(sourceBoardItemId: string, targetBoardItemId: string) {
	const input = MergeBoardItemsInputSchema.parse({
		sourceBoardItemId,
		targetBoardItemId,
	});
...
export async function produceBoardItem(
	boardItemId: string,
	activation: "single" | "exhaust" = "single",
): Promise<ProducerDropResult> {
	const input = ProduceBoardItemInputSchema.parse({
		boardItemId,
		activation,
	});
	return db.transaction().execute(async (tx) => {
		...
		if (producer.trigger !== "click")
			throw new GameActionError("This producer runs by itself.");
...
export async function buildRecipe(recipeId: string, x: number, y: number) {
	const input = BuildRecipeInputSchema.parse({
		recipeId,
		x,
		y,
	});
	await db.transaction().execute(async (tx) => {
		...
		assertInsideBoard(save, input.x, input.y);
		if (boardRows.some((row) => row.x === input.x && row.y === input.y))
			throw new GameActionError("Build target is occupied.");
