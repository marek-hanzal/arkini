...
Material `consume` is destructive conversion, not ordinary identity removal. Before start, the material remains an ordinary input item with all owned state intact. At authoritative start, discard its complete owned subtree without returning anything, commit the consumed root to the active job, and remove that root only at completion. `reserve` instead preserves state and returns the material through standard placement. Both modes are one atomic candidate and failed later start checks must restore the pre-start ownership tree exactly.
