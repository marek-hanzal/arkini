...
export function assertStartingState(config: GameConfig, itemIds: Set<ItemId>) {
  assert(config.startingState.inventory.length <= config.game.inventory.slots, "Starting inventory has more stacks than available slots");
  ...
  for (const boardItem of config.startingState.board) {
    ...
    assert(boardItem.x >= 0 && boardItem.y >= 0 && boardItem.x < config.game.board.width && boardItem.y < config.game.board.height, `Starting board item ${boardItem.itemId} is outside the board`);
