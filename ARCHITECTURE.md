...
External authored JSON and Resource catalog changes do not refresh mounted Editor projections. Project projections hold resource metadata, never binary bodies; item/config saves write only changed JSON and the revision marker. Requested image and music previews stream current files from their registered native paths; Music may request byte ranges. Already mounted images, audio and Editor Board are not watched. Explicit Refresh settles writes, discards drafts and Editor Board, rereads the directory metadata and publishes one replacement. Build reads and validates the saved source tree independently of that projection. There is no watcher, merge, repair mode, partial load or second project store. MCP uses the same repository, schemas and revision checks.
