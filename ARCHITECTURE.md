...
Editor item estimates use one explicit three-layer boundary. The canonical engine owns every gameplay transition and remains the sole authority for inputs, jobs, reservations, charges, lifecycle, merges, output resolution, and the resulting immutable runtime snapshot. A planner strategy owns only the decision algorithm over those transitions: the global best-first strategy maintains a runtime frontier, while the constructive strategy maintains goals, speculative snapshots, and backtracking. A strategy cannot mutate the live game, publish editor DTOs, or replace an engine rule with a parallel economic implementation.

`Planner` is the sole public orchestration capability. It owns one immutable planner kernel for the authored config, creates a fresh Effect-native planning session for every estimate, invokes exactly one configured root strategy, validates the returned engine witness, and turns that trace into expected economics. The session provides the shared global budget, current-strategy identity, invocation diagnostics, and recursive `solveSubgoalFx` routing. Simulated worlds remain immutable branch-local values; session services coordinate search work but never become a second gameplay store. Delegation depth belongs exclusively to the branch-local `PlannerCurrentStrategyFx` invocation context, while `PlannerProblem` owns only the immutable world plus a normalized active-first goal agenda.

A root strategy may be a leaf algorithm or a composite strategy. `AdaptivePlannerStrategy` wraps any number of child strategies and deterministically selects one from the current immutable runtime, active goal, agenda, and session context. A child that discovers a new subgoal delegates it back through `PlannerSessionFx.solveSubgoalFx`; the request therefore returns to the same root adaptive strategy, which may choose a different child for that new situation. Callers never pass a strategy list or fallback plan, and there are no historical strategy-specific planner entry points. Scheduling may later become concurrent or worker-backed, but strategy selection, global budgets, engine validity, and normalized planner results remain independent of wall-clock completion order.
