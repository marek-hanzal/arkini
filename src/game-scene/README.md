...
- A layer change cancels the current gesture and its magnetic feedback. The surface fades the outgoing front band out, swaps painter order while it is transparent, then restores it behind the selected band. Rapid reversal starts from current opacity. Pointer hits resolve to the canonical preferred occupant even before painter order catches up; absent preferred occupants fall back to the other layer. The same priority selects drop targets and magnetic candidates, and is frozen as `interactionLayer` in each submitted engine command. Layer selection never changes definitions, placement, production, or saved runtime state.
