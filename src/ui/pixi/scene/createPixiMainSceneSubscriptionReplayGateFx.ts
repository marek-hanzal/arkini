 ...
 */
export const createPixiMainSceneSubscriptionReplayGateFx = Effect.fn(
	"createPixiMainSceneSubscriptionReplayGateFx",
)((hydratedSequence: number) =>
	Effect.sync((): PixiMainSceneSubscriptionReplayGate => {
		let awaitingInitialReplay = true;

		return {
			classifyFx: Effect.fn("PixiMainSceneSubscriptionReplayGate.classifyFx")((sequence) =>
				Effect.sync(() => {
