...
describe("useEditorNavigationBlocker", () => {
	it("keeps a clean project routed until refresh finishes durable and renderer replacement", async () => {
		const fixture = await createFixture();
		const pending = fixture.rendererRuntime.runPromise(
			refreshEditorProjectFx({
				projectId: fixture.project.projectId,
				isNavigationPendingFn: () => fixture.router.state.status === "pending",
			}),
		);
		try {
			...
			expect(fixture.refreshProjectFn).toHaveBeenCalledOnce();
	...
	it("rejects refresh while an already admitted project navigation is loading", async () => {
