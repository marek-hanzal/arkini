...
MCP `rename_item` selects an exact `itemUid` and changes only `title`. Artwork resource identities remain independent and are changed through resource authoring. Writes retain the revision-guarded project lock and atomic individual replacement contract, without aggregate rollback.
