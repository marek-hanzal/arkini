...
export async function ensureCrossOriginIsolation() {
  ...
  if (!("serviceWorker" in navigator)) {
    throw new Error("This browser cannot enable cross-origin isolation without server headers.");
  }
  ...
  const serviceWorkerUrl = new URL(`${import.meta.env.BASE_URL}coi-serviceworker.js`, window.location.href);
