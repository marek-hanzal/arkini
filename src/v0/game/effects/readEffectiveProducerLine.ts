...
import type {
	...
	EffectiveProducerLineOutputEntry,
	EffectiveWeightedProducerLineOutputSubEntry,
...
export namespace readEffectiveProducerLine {
...
type ProducerLineOutput = NonNullable<GameProducerLineDefinition["output"]>[number];
type NonWeightedProducerLineOutput = Exclude<
	ProducerLineOutput,
	{
		type: "weighted";
	}
...
>;
type WeightedProducerLineEntry = WeightedProducerLineOutput["entries"][number];
type ProducerProducerLineDropEffect = NonNullable<NonWeightedProducerLineOutput["effects"]>[number];
...
type RuntimeItemSelector = Extract<
	ProducerProducerLineDropEffect,
	{
...
const readLineEffectLabel = ({
	...
	config: GameConfig;
	fallback: string;
	lineEffect: ProducerProducerLineDropEffect;
}) => {
...
const createDropEffectOutcome = ({
	...
	active: boolean;
	effect: ProducerProducerLineDropEffect;
	effectId: string;
	effectName: string;
...
const readEffectiveDrop = ({
	...
	dropEffects: readonly ProducerProducerLineDropEffect[] | undefined;
	enabled?: boolean;
	grantIds: ReadonlySet<string>;
	itemId: string;
	save: GameSave;
...
const readEffectiveOutputEntries = ({
	...
	const appliedEffects: AppliedGameEffectOperation[] = [];
	const rollableOutput: EffectiveProducerLineOutputEntry[] = [];
	const visibleOutput: EffectiveProducerLineOutputEntry[] = [];
	...
	for (const [outputIndex, entry] of output.entries()) {
		if (entry.type === "weighted") {
			const visibleEntries: EffectiveWeightedProducerLineOutputSubEntry[] = [];
			const rollableEntries: EffectiveWeightedProducerLineOutputSubEntry[] = [];
...
export const readEffectiveProducerLine = ({
	baseDurationMs,
	config,
	...
	lineId,
	save,
}: readEffectiveProducerLine.Props): EffectiveProducerLine => {
