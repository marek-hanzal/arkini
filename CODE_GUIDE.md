...
Arkini is the presentation source of truth. It owns actor phase, operation generation, source/target identities, topmost `slot | surface | outside` hit testing, z-order, engine handoff, and current placement. Motion owns only direct pointer drag tracking through the actor's local `x/y` values. Do not derive gameplay or presentation truth back from Motion, and do not drive pointer-frequency React renders. Pointer press only arms the interaction. Crossing the drag threshold moves the one live actor directly with the pointer; release resets its local offset immediately.

Every release invokes one public atomic engine drop command. UI must never infer merge, consume, swap, stacking, storage eligibility, placement fallback, or removal semantics. While the command is pending, the exact released source and target facts stay frozen for command ownership; when it finishes, that generation clears and the actor layer shows the newest canonical bridge snapshot immediately. Rejected and unsupported targets are truthful feedback, not animation stages. The tile system currently has no movement interpolation, pickup correction, crowd response, producer emission, scale/fade effect, retained exit, impact, or post-drop choreography. New choreography must consume exact standalone engine facts without moving gameplay truth into presentation.
...
Local visual lifecycles may own explicit Arkini-authored presentation states, but Motion is the sole local animation runtime. Active UI code must not call `element.animate()`, construct `Animation` / `KeyframeEffect`, or coordinate interruptible interaction feedback through CSS transitions or keyframes. Presentation never decides whether save, reset, route release, or shutdown succeeded. Keep the authoritative operation fully visible while pending; start a closing transition only after success, retain pointer/focus authority until actual completion, and leave failures in their truthful settled scene. Do not build a generic animation manager merely because two independent surfaces animate.
