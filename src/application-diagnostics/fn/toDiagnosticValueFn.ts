...
const toUnreadableDiagnosticValueFn = (cause: unknown): DiagnosticValue => {
	try {
	...
	try {
		...
		if (value instanceof Error) {
			...
			if ("cause" in value && value.cause !== undefined) {
				error.cause = toDiagnosticValueAtDepthFn(value.cause, depth + 1, seen);
			}
			if (value instanceof AggregateError) {
				error.errors = Array.from(value.errors)
					.slice(0, 20)
					.map((cause) => toDiagnosticValueAtDepthFn(cause, depth + 1, seen));
			}
			for (const key of Object.keys(value).slice(0, 100)) {
				...
				try {
					...
					error[key.slice(0, 100)] = toDiagnosticValueAtDepthFn(
						(value as unknown as Record<string, unknown>)[key],
						0,
						seen,
					);
				} catch (cause) {
					error[key.slice(0, 100)] = toUnreadableDiagnosticValueFn(cause);
				}
			}
			return error;
		}
		if (Array.isArray(value)) {
			return value
				.slice(0, 100)
				.map((entry) => toDiagnosticValueAtDepthFn(entry, depth + 1, seen));
		}

		const result: Record<string, DiagnosticValue> = {};
		for (const key of Object.keys(value).slice(0, 100)) {
			try {
				result[key.slice(0, 100)] = toDiagnosticValueAtDepthFn(
					(value as Record<string, unknown>)[key],
					depth + 1,
					seen,
				);
			} catch (cause) {
				result[key.slice(0, 100)] = toUnreadableDiagnosticValueFn(cause);
			}
		}
		return result;
	} catch (cause) {
		return toUnreadableDiagnosticValueFn(cause);
	}
};
