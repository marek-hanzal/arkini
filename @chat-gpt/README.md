...
- Choose The Path is now represented by three Era XI keystone building items: `producer:house-of-engineers`, `producer:cathedral`, and `producer:mage-lodge`. University issues their blueprints after prestige/treasure inputs. The buildings are mutually exclusive through `exclusiveToIds` and emit passive global effects that block University from producing counter-path blueprints. Later branch products should be baseline-hidden and revealed by path effects for branch-tagged variants.
