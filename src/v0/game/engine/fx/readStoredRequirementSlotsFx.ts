...
export const readStoredRequirementSlotsFx = Effect.fn("readStoredRequirementSlotsFx")(
	function* ({ config, save, targetItemInstanceId }: readStoredRequirementSlotsFx.Props) {
		...
		const storedRequirements = requirements.filter(
			(requirement): requirement is Extract<GameRequirement, { type: "stored" }> =>
