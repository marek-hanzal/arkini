...
import { checkEditorMcpPortAvailabilityFx } from "../../../../electron/main/editor-mcp/http/checkEditorMcpPortAvailabilityFx";
...
describe("checkEditorMcpPortAvailabilityFx", () => {
	it.each([
		1_023,
		65_536,
		32_310.5,
		...
		expect(await Effect.runPromise(checkEditorMcpPortAvailabilityFx(candidate))).toEqual({
			type: "unavailable",
	...
	it("reports a released loopback port as available", async () => {
		...
		expect(await Effect.runPromise(checkEditorMcpPortAvailabilityFx(port))).toEqual({
			type: "available",
		});
	});
