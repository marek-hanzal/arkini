...
[`space/resolveInventoryFx`](../space/fx/resolveInventoryFx.ts) owns lazy allocation and initialization in the caller's unpublished draft. Runtime Item and State Item carry the single optional `inventory` binding; existing `templateUidBySpace` owns active geometry. There is no separate ownership registry. Allocation reserves every authored literal and occupied current/snapshot address. Outcome application passes the exact definition owner; receiver transport passes its target. A dead owner cannot create an Inventory Space; repeated use returns the bound address. Same-ID replacement explicitly preserves the binding.
