...
const renderNotes = async (global = false) => {
	const component = global
		...
		: ArtworkNotesRoute.options.component;
...
const selectLinkFn = async (container: HTMLElement, kind: "Item" | "Resource", title: string) => {
	await clickFn(
...
describe("resource Notes", () => {
	it("retains mixed links through failed filtered creation, preserves search, and removes only the current resource link", async () => {
		state.createFailures = 1;
		const container = await renderNotes();
		expect(
			container.querySelector<HTMLButtonElement>('[data-ui="EditorNoteUnlinkResource"]')
				?.disabled,
		).toBe(true);
		await setContentFn(container, "Art direction");
		await selectLinkFn(container, "Resource", "asset-wood");
		...
		expect(container.querySelectorAll('[data-ui="EditorNoteResourceLinks"] a')).toHaveLength(2);
		await createNoteFn(container);
		...
		const preview = container.querySelector<HTMLAnchorElement>(
			'[data-ui="EditorNote"] [data-ui="EditorNoteResourceLinks"] a',
		);
		expect(preview?.getAttribute("href")).toBe(
			"/editor/project-one/artwork/asset-water/detail/overview",
		);
		...
		await clickFn(
			container.querySelector('[data-ui="EditorNote"] [data-ui="EditorNoteUnlinkResource"]'),
		);
	...
	it("keeps missing resource relationships visible and removable without a dangling navigation target", async () => {
		state.notes = [
			{
				noteId: "missing",
				projectId: "project-one",
		...
		const container = await renderNotes(true);
		expect(
			container.querySelector('[data-ui="EditorNoteMissingResource"]')?.textContent,
		).toContain("missing-asset");
		expect(
			container.querySelector('[data-ui="EditorNote"] [data-ui="EditorNoteResourceLinks"] a'),
		).toBeNull();
		await clickFn(
			container.querySelector('[data-ui="EditorNote"] [data-ui="EditorNoteUnlinkResource"]'),
		);
