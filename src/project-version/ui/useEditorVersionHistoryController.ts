...
} from "~/project-version/fn/layoutEditorVersionGraphFn";
import { useEditorVersionCheckout } from "~/project-version/ui/useEditorVersionCheckout";
import { useEditorVersionComparison } from "~/project-version/ui/useEditorVersionComparison";
import { useEditorVersionTag } from "~/project-version/ui/useEditorVersionTag";
...
export const useEditorVersionHistoryController = (): useEditorVersionHistoryController.Output => {
	...
	const graph = useMemo(
		() =>
			history === undefined
				? undefined
				: layoutEditorVersionGraphFn(history.versions, history.status.currentBaseVersionId),
		[
			history,
		],
	);
