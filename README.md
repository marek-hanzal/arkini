...
MCP `graph_schema_json` documents the focused graph tools and selective hydration. `graph_search` finds titled nodes by name or identity; `graph_connections` reads direct relationships; `graph_operations` lists, counts or groups authored operations with participant, search and scalar filters. `graph_path` finds structural paths, while `graph_flow` lists directed paths through authored operations with explicit rules and stochastic facts, without evaluating gameplay feasibility. `graph_traverse` explores broader structural neighborhoods and requires explicit edge kinds beyond one hop. `graph_batch` runs up to eight named queries on one snapshot, with independent statuses and presentation. Discovery returns compact text with short operation references, exact node IDs and snapshot pins. `graph_audit` answers factual questions about disconnected content, authored producers, usage and configured behavior, with independent template/reference facts, exact operation references and count mode. It does not evaluate gameplay rules or label design errors. Connections, operations and audits support compact continuation. `graph_operations_json` hydrates selected operation references; `items_json` and `item_lines_json` provide complete item/line documents. See the [graph contract](src/graph/README.md).

MCP Board Templates have focused collection/detail reads. `template_detail` shows occupied/free cell counts and a zero-based ASCII board (X right, Y down) with separate symbols and exact item identities for every placement. Maps over 10,000 cells are explicitly omitted, with the complete placement legend retained. Templates have canonical JSON through `template_json`, creation, patching, deletion and ordered cell edits. All mutations use the project revision; descriptions point to exact input schemas. See [Template authoring](CONFIG.md#mcp-board-template-authoring) for the workflow and cell-operation contract.
