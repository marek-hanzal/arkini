...
export const readOutputHighlightFx = Effect.fn("readOutputHighlightFx")(
		...
		Effect.sync((): Highlight => {
			...
			for (const [, directOutputEdges] of directBranches) {
					...
					const witness =
						itemNode.acquisitionSourceId === undefined
							...
							: directOutputEdgesForItem.find(
									({ operationId }) =>
										operationId === itemNode.acquisitionSourceId,
								);
				...
				const traceOperation = (
					...
					const ownerNode =
						ownerNodeId === undefined ? undefined : nodeById.get(ownerNodeId);
					...
					for (const edge of [
						...
						if (requirementNode !== undefined)
							traceItem(requirementNode, activeItemIds);
					}
				};
