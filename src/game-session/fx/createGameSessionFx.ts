...
import type { GameSession, GameSessionServices } from "~/game-session/type/GameSession";
import {
	GameSessionFatalError,
	type GameSessionFatalSource,
} from "~/game-session/error/GameSessionFatalError";
import { GameSessionNotRunningError } from "~/game-session/error/GameSessionNotRunningError";
import {
	type GameSessionTransitionSubscriptionCleanup,
	createGameSessionTransitionSubscriptionsFx,
} from "~/game-session/fx/createGameSessionTransitionSubscriptionsFx";
...
const createGameSessionFatalSignalFx = Effect.fnUntraced(
	function* (): Effect.fn.Return<GameSessionFatalSignal> {
			...
			Effect.gen(function* () {
				...
				const initializeFx = Effect.gen(function* () {
					...
					const read = <Result, ReadError, Requirements extends GameSessionServices>(
						...
						// Initialization above has already made this ManagedRuntime hot, so its
						// construction failures cannot enter a live read's error channel.
						return managed.runSyncExit(effect) as Exit.Exit<Result, ReadError>;
					};
