...
export namespace readPlannerStructuralRuntimeIndexFx {
...
const readRouteRuntimeMs = ({
	...
	Effect.gen(function* () {
		...
		const witnessRuns = yield* readPlannerExpectedIndependentRunsFx({
 ...
 */
export const readPlannerStructuralRuntimeIndexFx = Effect.fn("readPlannerStructuralRuntimeIndexFx")(
	function* ({ config, graph }: readPlannerStructuralRuntimeIndexFx.Props) {
		...
		for (let pass = 0; pass < maximumPasses; pass += 1) {
			...
			for (const route of graph.routes) {
				...
				const candidate = yield* readRouteRuntimeMs({
					config,
					route,
					runtimeByItemId,
				});
