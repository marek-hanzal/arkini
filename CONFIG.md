...
- material selectors may name any canonical item, including Clock identities whose interval and lifetime pause in input and job storage;
...
`src/artwork-authoring` owns the Artwork catalog, square-PNG admission, import/edit/delete sessions, Optimize, and presentation. Project Images owns general-PNG import and launcher/shell mappings. `src/audio-authoring` owns shared Music/SFX library import, lazy one-track preview and canonical Ogg/Opus preparation; noncanonical input conversion depends only on a PATH-visible `ffmpeg`. `src/music-authoring` owns random-playlist selection over that shared audio surface, while `src/sfx-authoring` owns gameplay-event assignment over the Sound-effects workspace. `src/resource-authoring` owns shared typed import orchestration; `src/authoring-session` owns mounted-project resource URLs; `src/authoring-form` owns the typed Resource-reference control. `src/game-config-resource` owns the generic Resource schema, semantic type, exact source discovery, PNG and Ogg/Opus admission, references, usage, and rename semantics. Explicit non-item Image roles, the global Music playlist, and exact gameplay-event SFX assignments belong to the completed `src/game-config` value they populate.
