...
import type { InventoryPlacementPlan, PlacementPlan } from "~/inventory/logic/planning";
...
export async function applyPlacementPlan(tx: ArkiniTransaction, plan: PlacementPlan): Promise<ProducerPlacement[]> {
  ...
  for (const placement of plan.inventory) {
    if (placement.type === "update") {
      await tx.updateTable(table.inventoryStack).set({ quantity: placement.quantity, updatedAt: localTimestamp() }).where("id", "=", placement.stackId).execute();
    } else {
      await tx
