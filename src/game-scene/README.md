...
- Board right click runs the primary action; `Ctrl+right click` fills remaining default-line queue capacity; `Shift+right click` splits a Board stack; left click opens Item Detail. Portals retain left-click activation and right-click Detail. Item dragging remains on the left button and camera dragging on the right.
...
- During manual drag, the hovered target artwork shrinks and fades to 0.8 for rejected drops or swaps, then returns on target change or gesture end. Merge, stack and input acceptance keep their normal appearance. This response has its own animation channel and never changes placement, hit geometry, lifecycle or running opacity.
- The Engine drop preview owns validity. Pixi geometry never infers merge, stack, input, swap, or placement behavior.
