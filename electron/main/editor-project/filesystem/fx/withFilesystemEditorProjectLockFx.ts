...
const acquireLockFx = Effect.fn("acquireFilesystemEditorProjectLockFx")(function* (root: string) {
	...
	return yield* Effect.fail(
		new Error(
			`Editor project ${root} is locked by another process. If that process is gone, remove editor.lock and retry.`,
		),
	);
});
...
/** Serializes one cross-process write scope through the project's short-lived lock file. */
export const withFilesystemEditorProjectLockFx = <Value, Error, Requirements>(
	root: string,
