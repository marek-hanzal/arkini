...
export namespace readProducerLineRunState {
...
const readInputsPartiallyAvailable = (line: ProducerLineView) =>
	!line.inputsReady &&
	line.inputs.some((input) => readActivationInputViewFillableQuantity(input) > 0);

const readOutputsDisabled = (line: ProducerLineView) => {
...
const readStatusMetaLabel = (line: ProducerLineView) => {
...
const withCommonState = ({
	...
	label: string;
	line: ProducerLineView;
}): readProducerLineRunState.Result => ({
	canRunAction,
	inputsPartiallyAvailable,
...
export const readProducerLineRunState = ({
	line,
}: readProducerLineRunState.Props): readProducerLineRunState.Result => {
