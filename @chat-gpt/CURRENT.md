...
Status: **`v0` is the immutable pre-interaction backup of `4c25f6d2cd1aa3e9d0b2efc63f54abfc2e14d82f`; feature work continues on `main`. The accepted route-transition baseline and completed singleton/fatal/controlled-close lifecycle remain intact. Epic #302 supersedes the earlier manual ghost/WAAPI DnD attempt. Review root #310 is implemented and independently revalidated: tile interaction state is discriminated, every transition and settlement branch is compile-time exhaustive, `TileSystemProvider` is a small shell over focused geometry and transition owners, `TileActor` is markup-oriented over presentation/Motion/drag hooks, the actor layer retains only generation-owned removed visuals, Board surface identity is stable per current space, slot identity changes only with its surface/coordinate/occupant identity, and complex active UI has focused lifecycle/view-model hooks with exhaustive dynamic rendering. The current Board vertical slice still uses one Canvas-wide Motion actor layer, one real stable DOM actor per runtime item, pronounced `1.15×` preview hover, exact pointer drag, and one atomic engine drop command.**
...
- The first #320 hover-action slice is implemented. Every live Board/Toolbar actor owns one conditionally mounted Floating UI action strip anchored to its transformed drag surface, preferring `bottom` with a small negative overlap plus flip/shift collision handling and animation-frame auto-update. Arkini config explicitly maps Info, Status, Lines, and Effects icon resource IDs under `resources.tileCapabilities`; the UI resolves those images through the loaded Game's existing Arkpack `getResourceUrl()` boundary and renders them at 24×24. Hero and capability PNGs live under `game/arkini/resources`, while item artwork remains under `game/arkini/assets`. Only one strip remains visible when crossing actors, all strips are suppressed during `pressed`, `dragging`, `awaiting-outcome`, or an open tile workspace, and post-outcome `settling` remains hover-capable under the immediate-reinteraction contract. Retained/exiting non-live actors never expose actions.
- Info is the first completed capability vertical slice. Clicking Info freezes the exact runtime ID and originating actor, closes hover, and opens one Canvas-local viewport-constrained `TileWorkspace` over an unchanged Board/Toolbar. The workspace owns one generation-safe `closed | entering | open | exiting` lifecycle, backdrop dismissal, capture-phase Escape ownership, focus containment/restoration, stale-target close, and local Motion only. Every capability now renders through one shared compact tile chrome: a 64px layered Arkpack artwork, title, category, optional capability state, and close action. The broad Info projection is independent from that shell identity and exposes common authored/live facts only: description, item kind, category, humanized traits, current semantic location, allowed storage, current stack, stack capacity, total owned quantity, configured game limit, and remaining/authored charges. Exact item removal closes the workspace; slot replacement never retargets it.
- Status is the second capability slice and is exposed only by canonical line owners (`producer | blueprint | craft | stash`). The engine projection classifies one exact live owner as `ready`, `working`, `stored`, or `paused`; paused is explicitly split between passive Inventory/Toolbar storage and currently unsatisfied live Board dependencies. The bridge subscribes to that narrow immutable projection and React only exhaustively maps it to concise player copy. The same open modal updates in place across runtime publications and never remounts or retargets. Status remains owner-level only; line readiness, inputs, outputs, effective runtime, and active-job timing now live in Lines, while queue/output blockage and generic tile feedback remain later slices. #322 remains open because blocked/depleted presentation is not yet a valid or owned part of this narrow current-runtime contract.
- Effects is the third capability slice. Line owners expose current incoming line conditions, while non-line items expose Effects only when they currently participate in another owner's outgoing live condition. The shared workspace separates Incoming and Outgoing, keeps exact runtime identity, and updates in place. The first projection covers line visibility/enable/disable/runtime-multiplier conditions with matched live items, scope/distance, quantity, and active state. #323 remains open for the broader final semantic pass, especially grants/unlocks and any future canonical effect grammar that is not represented by current line rules.
- Lines now has its first read-only vertical slice under #324. Every canonical line owner exposes the fourth hover capability and reuses the shared workspace chrome. `readTileLinesFx` consumes existing `resolveLineRunFx` on Board owners, so visibility, enable state, effective runtime, and input readiness are the same facts used by canonical start planning. Stored owners remain inspectable but are explicitly blocked by storage rather than receiving invented spatial readiness. The projection groups repeated compatible material/deposit inputs, preserves authored output set/roll semantics, collapses duplicate drops only inside the same meaningful roll, and exposes active-job timing without mutation controls. React renders one flat scrollable line list with restrained dividers, not per-kind dialogs or nested card hierarchies. Start/enqueue, default line, Autofill, Withdraw, queue controls, line-effect summaries, and command errors remain later #324 slices.
...
> Continue #324 from the landed read-only Lines list. The next bounded slice should stabilize active-work and authoritative FIFO queue presentation inside the same modal before adding mutations. After that, add Start/enqueue plus default-line behavior, then separate Autofill and Withdraw commands, and finally Clear queue plus recoverable command errors. Reuse `readTileLinesFx` or a narrower sibling projection; do not mirror queue/input/job state in React, combine Autofill with Start, introduce per-kind line dialogs, or flatten output/effect semantics for prettier rows. Keep #323 open until grants/unlocks and the final semantic Effects scope are decided.
