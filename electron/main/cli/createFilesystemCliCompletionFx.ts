...
import type {
	CliCompletionShell,
	CliCompletionStatus,
} from "../../contract/cli/CliCompletionStatus";
...
export namespace createFilesystemCliCompletionFx {
...
export const createFilesystemCliCompletionFx = Effect.fn("createFilesystemCliCompletionFx")(
	function* ({
		completion,
		launcherPath,
		unavailableMessage,
	}: createFilesystemCliCompletionFx.Props) {
		...
		const managedFile = yield* completion === undefined
			...
			: createFilesystemCliManagedFileFx({
		...
		const readStatus = async (): Promise<CliCompletionStatus> => {
		...
		const operation = (name: string, run: () => Promise<CliCompletionStatus>) =>
			Effect.tryPromise({
				try: run,
				catch: (cause) =>
