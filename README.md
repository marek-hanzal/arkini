...
- the integrated editor stores each canonical project in the packaged renderer origin's persistent IndexedDB through one Dexie-backed Effect repository. Local form values are the only draft; an explicit form or asset Save commits one atomic project mutation. Import validates an Arkpack before one transactional project/resource insert, while Build performs the complete semantic/resource validation and creates immutable bytes that may independently be saved or installed into the Arkpack catalog. Those exact bytes are also the intended input for the next editor-only gameplay-preview flow. Editor projects never write source workspaces into the repository or signed application bundle.
