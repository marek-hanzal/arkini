...
| Area | Owner |
| --- | --- |
| Shared Board/Inventory camera, zoom and pan | `fx/createBoardCameraFx.ts` |
...
| Board layer selection and temporary Alt/Option hold | `fx/createBoardLayerControlFx.ts`, `fx/bindBoardLayerKeyboardFx.ts` |
...
- Crossing the drag threshold converts the same pointer gesture into drag. Neighborhood artwork blends to the ordinary item artwork while held; canonical occupancy remains at the source until the drop commits, and release or cancellation blends back to the latest projected artwork through the same guarded visual pipeline. The retained actor is reparented without a duplicate actor or pointer-frequency React render. On travel settlement into Ground or dimmed Content, a non-interactive snapshot fades out above the active layer for 220 ms. The animator releases the snapshot and texture on completion, new movement, or actor teardown; it never participates in gameplay or hit testing. Layer switching lets an existing tail finish its fade.
- Board actors retain two bands, with `content` active by default. `usePixiGameRuntime().boardLayer.setLayerFx(layer)` selects either band; `setGroundHeldFx` temporarily overrides that selection. Alt/Option is only the Board keyboard adapter, released on keyup, blur, hidden document, overlays, or unmount. Editing fields do not acquire the hold. Inventory and Toolbar remain single-layer.
- A layer change cancels the current gesture and its magnetic feedback. The surface keeps painter order stable and Ground fully opaque. Ground selection smoothly dims Content into a translucent ghost; returning restores Content without fading Ground. One overdamped opacity spring retains its current value and velocity across rapid reversals. Pointer hits resolve to the canonical preferred occupant independently of painter order; absent preferred occupants fall back to the other layer. The same priority selects drop targets and magnetic candidates, and is frozen as `interactionLayer` in each submitted engine command. Layer selection never changes definitions, placement, production, or saved runtime state.
...
- `readNeighborhoodArtworkFn` derives ordered same-layer Board artwork from committed occupants, never selected images or delivery leases. Tile projections retain ordinary artwork for the drag fallback. Held actors and subsequent restoration use the existing visual crossfade owner; dragging never changes the occupancy projection or previews neighborhood changes at hover targets.
