...
const outcomeFn = (templateUid: string) =>
...
it("rejects missing Template references from every item outcome owner before either write path publishes", async () => {
	const repository = await harness.openRepository();
	const project = await harness.createProject(repository);
	const outcome = outcomeFn("missing-template");
	const patches = [
		{
			lines: [
				createLine({
	...
	expect(await Effect.runPromise(repository.readProjectFx(project.projectId))).toEqual(project);
...
it("allows unrelated unfinished items, but blocks removing a referenced Template until its outcome is repaired", async () => {
	const repository = await harness.openRepository();
	...
	expect((await Effect.runPromise(repository.readProjectFx(project.projectId)))?.revision).toBe(
		saved.revision,
	);
