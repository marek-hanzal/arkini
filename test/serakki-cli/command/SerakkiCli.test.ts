...
beforeEach(async () => {
	root = await mkdtemp(join(tmpdir(), "serakki-signing-cli-"));
});
...
describe("Serapack provenance CLI", () => {
...
describe("game incident CLI", () => {
	it("replays a fixed incident environment through the production GameSession", async () => {
		...
		await writeFile(join(incident, GameIncidentFiles.serapack), createTestSerapack());
		await writeFile(
		...
		expect(result.stdout).toContain("# Serakki game replay");
	...
	it("reports invalid replay requests without framework stacks or physical paths", async () => {
		...
		expect(missingInputOutput).toContain("Could not read the replay Serapack");
		for (const output of [
...
describe("CLI completion", () => {
	it("generates static Bash, Fish, and Zsh completion from the public command tree", async () => {
		for (const shell of [
			...
			expect(result.stdout).toContain("serapack");
