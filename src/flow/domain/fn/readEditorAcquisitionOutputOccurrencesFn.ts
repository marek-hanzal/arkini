...
export const readEditorAcquisitionOutputOccurrencesFn = (output: OutputSchema.Type | undefined) => {
	if (output === undefined)
		return {
			occurrences: [],
				...
				for (const [candidateIndex, candidate] of roll.drop.entries()) {
					for (const [dropIndex, drop] of candidate.drop.entries()) {
						...
						readDrop(
							...
							setProbability * expectedSelections * (candidate.weight / totalWeight),
						);
					}
				}
			} else {
