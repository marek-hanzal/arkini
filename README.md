...
Client-only offline merge-game prototype. Plain Vite + React SPA, static-host friendly, no SSR, no server runtime, no HTTP API. Live gameplay state belongs to `RuntimeGameEngineAdapter` and `GameRuntimeStore`; persistence will wrap the runtime later instead of dictating its shape. Static gameplay rules are authored as JSON fragments, compiled into a validated gzip-compressed Arkini binary pack, and consumed by the standalone tick/action engine.
