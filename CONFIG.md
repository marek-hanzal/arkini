...
- each start entry creates one item in its exact Board cell; runtime items represent single identities, while authored input and output quantities count separate items;
...
- An item may author one optional `action` of type `space` with its target `space`, and optional `input` and `rules` collections defaulting to empty. Action rules alone determine click availability; no `enable` field exists. An occupied `space` action target is a directional portal drop target: it moves the exact source identity to the first free cell on the authored Board before input, merge or swap resolution, without activating the action or changing the current space;
...
- `units` defines a finite supply inside each item instance (health, resource stock, or uses), independently of authored input and output counts: passive and manually operated resources use ordinary items; scheduled production adds Item.clock;
