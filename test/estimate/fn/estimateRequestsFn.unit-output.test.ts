...
describe("estimateRequestsFn", () => {
	it("ignores unsupported multi-payer unit accounting", () => {
	...
	it("uses expected stochastic output runs without unit capacity", () => {
		const result = estimate(
			graph({
				facts: [
					"payer",
	...
	it("does not add spent payer instances to stochastic output timing", () => {
		const result = estimate(
			graph({
				facts: [
					"payer",
	...
	it("uses a faster nested stochastic route regardless of unit lifetime", () => {
		const result = estimate(
			graph({
				facts: [
					"payer",
