...
const readTestLine = ({
	...
	const [line] = readRuntimeProducerLineViewsFromGameSave({
		config,
		maxQueueSize: 1,
	...
	if (!line) throw new Error("Missing test producer line view.");
	return line;
};

describe("readRuntimeProducerLineViewsFromGameSave", () => {
	...
	it("keeps hidden producer lines visible while a runtime job still exists", () => {
		...
		const lines = readRuntimeProducerLineViewsFromGameSave({
			config,
			maxQueueSize: 1,
			producerDefinition: config.items["item:producer"].producer!,
			nowMs: 500,
	...
	it("surfaces activated effect polarity on effect producer lines", () => {
		const baseConfig = createEngineTestConfig();
		const config = createEngineTestConfig({
			effects: {
