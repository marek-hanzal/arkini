...
import type { EditorItemOriginFlowCommunities } from "~/ui/item/editor/readEditorItemOriginFlowCommunitiesFx";
import type {
	EditorItemOriginFlowLayoutProfile,
	EditorItemOriginFlowPair,
} from "~/ui/item/editor/readEditorItemOriginFlowTopologyFx";
...
export interface EditorItemOriginFlowPlacedNode {
...
export const placeEditorItemOriginFlowFx = Effect.fn("placeEditorItemOriginFlowFx")(
	(
		...
		{ communities, communityByNodeId }: EditorItemOriginFlowCommunities,
	) =>
		Effect.sync(() => {
			const types = [
			...
			try {
				return [
					...
					.sort((left, right) => left.id.localeCompare(right.id))
					.map((node): EditorItemOriginFlowPlacedNode => {
						const profile = profiles.get(node.id);
						if (profile === undefined)
							throw new Error(`Missing flow layout profile for ${node.id}.`);
