...
- `src/item-interaction` owns captured drop intent/results, optimistic preview, commit-time identity/location/capacity revalidation, move/stack/swap/store commit leaves, Inventory storage/release, stack split, explicit-removal transitions, and the ordinary single-click action read consumed by Pixi actors. Preview is advisory; every serialized commit revalidates against the current Runtime and publishes only one complete transition. Placement and merge execution stay in their exact upstream owners, production commands stay with production, and Item Interaction never owns Runtime schemas/store/checks, immutable Item definitions, Tick, presentation, routes, or Electron.
