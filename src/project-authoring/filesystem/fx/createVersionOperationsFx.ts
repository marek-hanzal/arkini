...
const createProjectFilesFn = (project: Project): ProjectFiles => ({
	arkpack: project.version,
	marker: {
						...
						const snapshot = yield* withProjectLockFx(
							...
							Effect.gen(function* () {
								...
								yield* providePlatformFx(
									writeProjectFilesFx({
										root: current.state.paths.root,
										previous: createProjectFilesFn(current.state.project),
										next: createProjectFilesFn(nextProject),
										...(preview.bump === "major"
											? {
