...
import {
	inputCue,
	stackCue,
} from "./readPixiTileQuantityPresentationFx.test/fixture";
...
describe("readPixiTileQuantityPresentationFx", () => {
	it("does not reveal an older stack through a later input epoch", () => {
		expect(read([stackCue, inputCue]).get("runtime:item")).toEqual({
			kind: "exact",
			quantity: 5,
		});
		expect(read([inputCue]).get("runtime:item")).toEqual({
			kind: "exact",
			quantity: 6,
		});
		expect(read([inputCue], new Set(["2:0"])).get("runtime:item")).toEqual({
			kind: "exact",
			quantity: 4,
		});
	});
