...
/** Commits one complete MCP project-json candidate against the caller's read revision. */
export const commitProjectConfigFx = Effect.fn("commitProjectConfigFx")(function* ({
	...
	if (revision !== undefined && revision !== project.revision)
		return yield* Effect.fail(
			new Error(
				`Revision ${revision} is stale; the open project is at revision ${project.revision}. Read project_json again before editing the project.`,
			),
		);
