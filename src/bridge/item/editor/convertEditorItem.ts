 ...
 * Target-only fields use the exact same defaults as a freshly created item.
 */
export const convertEditorItem = (item: EditorItem, targetType: EditorItemType): EditorItem => {
	if (item.type === targetType) return item;
	const fallback = createEditorItemDraft({
