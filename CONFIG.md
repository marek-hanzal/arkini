...
- optional item `clock` groups optional `intervalMs`, optional `durationMs`, optional `expiryMode`, `enable`, and `rules`. At least one timer is required. Lines may be empty. Both timers advance for the exact runtime identity while Clock rules permit time, including while an item is owned by input, a job, or a delivery. A periodic pulse outside Board cannot admit Board-owned production or create catch-up work. Its authored `enable` defaults to true; missing duration means unlimited active lifetime. Missing interval gives a one-shot lifetime without pulses. The Editor exposes both timers as optional fields; lifetime outcomes are authored on `clock-lifetime` lines;
- `clock.expiryMode` is `loose-kill` (also the meaning of omission) or `kill-switch`, and matters only with a finite lifetime. Loose-kill waits for accepted production and a selected lifetime Job to finish; blocked placement keeps that Job ready for retry. Kill-switch cancels earlier queued and active work, then starts the selected lifetime Job. After its authored runtime, its output and owner removal settle together, discarding any output or returned material that cannot fit. Consumed inputs remain consumed, spent units are not refunded, and canceled jobs emit no line outcome. Incoming deliveries return through their ordinary return path. Jobs already completed at the same Tick boundary retain their completion precedence. Without an eligible lifetime Job, a Board owner expires without line output. An item held in another job, input or delivery expires immediately with its selected lifetime-line outcome from its physical origin, since it cannot start a Board Job;
...
- line `default` selects one manual line; optional `clock` uses `LineClockModeEnumSchema` with `clock-interval` and `clock-lifetime`. `clockWeight` is an integer from 1 to 999, default 1. Editor Default selection clears sibling Default flags. Interval pulses draw one enabled line by weight after rule filtering and enqueue it only when its inputs are available in the line or on the Board. A missing input or rejected admission consumes the pulse without queueing work. At finite lifetime expiry, one eligible lifetime line is drawn by weight. A Board owner enqueues it when its inputs are available and runs for its authored production time before outcome settlement; an internally held item skips that line's inputs and runtime and settles the selected outcome immediately. Lifetime lines cannot be Default;
