...
Desktop delivery has one concrete Effect-owned sequence: clean output → pack the official game and build Electron once → stage the dependency-free application → invoke `electron-builder` once → stream DMG/ZIP SHA-256 values once → verify artifact and unpacked-app structure. The standalone verify command deliberately re-streams artifacts because it validates downloads or later changes; the combined package operation never rehashes files it just hashed. CI runs non-build gates before this same package command and permanent tests afterward, so hidden npm lifecycle hooks do not repeat the official pack or renderer build.
