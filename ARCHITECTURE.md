...
`Planner` is the sole public orchestration capability. It owns one immutable planner kernel for the authored config, creates a fresh Effect-native planning session for every estimate, invokes exactly one configured root strategy, validates the returned engine witness, and turns that trace into expected economics. The session provides the shared global budget, current-strategy identity, invocation diagnostics, and recursive `solveSubgoalFx` routing. Simulated worlds remain immutable branch-local values; session services coordinate search work but never become a second gameplay store.
