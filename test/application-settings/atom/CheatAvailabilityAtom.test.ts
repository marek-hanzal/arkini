...
import {
	applyCheatAvailabilityFx,
	awaitCheatAvailabilityFx,
} from "~/application-settings/fx/applyCheatAvailabilityFx";
...
describe("Cheat availability Atom", () => {
	it("completes readiness on the first apply and publishes changed values only", async () => {
		...
		let ready = false;
		const readiness = Effect.runPromise(awaitCheatAvailabilityFx).then(() => {
			ready = true;
		});
