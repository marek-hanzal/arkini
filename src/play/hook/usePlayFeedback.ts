...
export function usePlayFeedback() {
	...
	const flashBoardCell = useCallback((key: string | null, tone: "error") => {
		...
		window.setTimeout(
			() => setInvalidBoardCellKey((current) => (current === key ? null : current)),
			flashMs,
		);
	}, []);
	...
	const pulseMergeCell = useCallback((key: string | null) => {
		...
		window.requestAnimationFrame(() => {
			...
			window.setTimeout(
				() => setMergedBoardCellKey((current) => (current === key ? null : current)),
				560,
			);
		});
	}, []);
	...
	const flashInventorySlot = useCallback((slotIndex: number | null, tone: "error") => {
		...
		window.setTimeout(
			() => setInvalidInventorySlot((current) => (current === slotIndex ? null : current)),
			flashMs,
		);
	}, []);
