...
Desktop delivery has one concrete Effect-owned sequence: clean output → pack the official game and build Electron once → stage the dependency-free application → invoke `electron-builder` once → stream DMG/ZIP SHA-256 values once → verify artifact and unpacked-app structure. The standalone verify command deliberately re-streams artifacts because it validates downloads or later changes; the combined package operation never rehashes files it just hashed. CI runs format, type, and source-validation gates before this same package command, then runs Dependency Cruiser and permanent tests against the generated package inputs. The package command remains the only delivery owner of the official pack and renderer build, while fresh checkouts never require stale ignored output.
