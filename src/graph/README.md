...
Merge discovery identifies an explicit target, replacement and action/effect, with `hasOutcomes` reporting actual authored outcome entries. Receiver-owned Space transport identifies the owner as receiver and carries its destination. Referenced owners, targets, replacements and destinations are accompanied by titled nodes even when their edges were outside the selected query. Line summaries expose `lineUid`, so owner item UID plus line UID can feed `item_lines_json`. IDs remain opaque; clients must not decode positional operation IDs to address source arrays.

## MCP presentation

Tools without a format suffix return readable text. `graph_query` presents operation queries as operation rows, connection/traversal queries as directed relationships, and path queries as ordered paths. Names are primary; exact node, edge and operation identities remain available for follow-up calls. A merge row can directly explain owner + target → replacement, action/effect, receiver ownership and additional outcomes, without a redundant node table. Lines retain their immutable UID.
...
`graph_query_batch` takes 1–8 uniquely named `{id, query}` entries. Each `query` must be an object; its fields are validated independently so malformed query fields remain per-entry errors. All subqueries run against one captured immutable snapshot. Internally, node, edge and operation collections are deduplicated by identity and each query keeps references to its own records. MCP presents common project/revision/snapshot metadata once, then a readable section per query ID with its own status, truncation, reasons, paths and continuation; it does not expose the storage tables. A failed subquery reports its own error without replacing successful siblings. Batch revision/snapshot admission fails before any subquery when the requested snapshot is stale.

`graph_operations_json` accepts 1–20 operation IDs and requires the discovered revision **and** snapshotId. It returns only those canonical operation records, deduplicates repeated IDs and reports missing ones. No persistent operation UID is needed: content-bound snapshot admission prevents positional merge identities from pointing at another operation after a reorder, including external changes that retain the revision marker. Snapshot tokens are session-local; after restart, rediscover. Existing `items_json` and `item_lines_json` remain the item/line hydration layer and return their own revision, which clients must compare before combining reads.
