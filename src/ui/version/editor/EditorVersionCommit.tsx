...
export const EditorVersionCommit = () => {
				...
				<div className="grid gap-4 rounded-2xl border border-line bg-surface-raised/60 p-5">
					<label className="grid gap-1.5 text-sm">
						...
						<input
							...
							onChange={(event) => controller.setSubject(event.currentTarget.value)}
						/>
					...
					<label className="grid gap-1.5 text-sm">
						...
						<textarea
							...
							onChange={(event) => controller.setBody(event.currentTarget.value)}
						/>
					</label>
					<label className="grid gap-1.5 text-sm">
						...
						<input
							...
							onChange={(event) => controller.setTag(event.currentTarget.value)}
						/>
						<span className="text-xs text-subtle">
					...
					{controller.status?.canCommit === false ? (
						<p className="rounded-lg bg-surface p-3 text-sm text-muted">
							Working copy matches its current base. Change the saved project before
							committing.
						</p>
					) : null}
