...
- Item `ui` is `simple` or `default` (default `default`). Simple shows only item information, hiding the entire section navigation; Default always exposes all Item Detail tabs even with no production lines. Simple also denies player production commands, input management, queue changes and line selection; Default permits ordinary manual production. Clock, Action, movement and merge policies remain independent;
