...
- Item Detail contains one shared header, one stable tab row, and one internally scrolling body. `info` is always available. `lines` and `queue` are exposed only by the authoritative engine capability result. Unsupported requested tabs fall back deterministically to `info`; tab switching preserves the same modal shell and target.
