...
resources/<id>.png
notes/<noteId>.json
paintings/<paintingId>.json
```

Only `game.json`, `items/<type>/<uid>.json`, `assets/*.png`, and `resources/*.png` are game sources. Project metadata, Notes, painting recipes, locks, temporary files, and ignored `build/` artifacts are never compiled. Editor Build and `arkini-cli game pack` validate and build the current saved sources directly.
...
Tile Painter stores independent non-destructive recipes in `paintings/<encoded paintingId>.json`. Each recipe retains ordered repeating-texture layers and alpha-mask brush dabs, the scattering catalog and exact placed stamps, an optional reference image, a saved Preview grid, and references to canonical project Assets. New layers are appended on top with a full-canvas reveal stroke as their initial mask coverage. Erase is the default session tool; authors cut away coverage to expose lower layers, and Reveal restores it. Layer creation, including its initial coverage, is one undo step. Reveal/hide strokes gradually add/remove mask coverage; smooth strokes blend neighboring mask alpha only inside their brush imprint. The session brush can target the active layer or every texture layer, including hidden layers, to establish or erase a shared silhouette. One gesture snapshots its target layers and commits their masks as one undo step. Scattering remains independent of this target setting. All painter image selection uses existing project Assets; there is no separate file-import channel. Each image entry stores only its local image identity, label and canonical `sourceResourceId`. Recipes contain no image bytes or copied source files. Canvas and Preview resolve current project Assets, and every bake renders current source bytes. Resource rename atomically rewrites recipe source links and matching output IDs while advancing recipe freshness. Resource deletion rejects remaining semantic painting references; unused image entries do not prevent deletion. The file records its freshness timestamp and optional baked output resource ID; the encoded filename owns painting identity, while project identity comes from the containing project. The fixed canvas and PNG output are 1254 × 1254 px; dimensions are not stored as redundant document fields. Bounds are 16 layers, 128 source images, 500,000 mask dabs, 100,000 scatter stamps, 64 MiB per recipe, and 256 recipes per project. Source images must be single-frame PNGs no larger than 2048 × 2048 px.

Each texture layer can customize its shadow color, strength, blur and horizontal/vertical offset. An omitted `shadow` setting uses the shared enabled default; disabling retains the chosen parameters. The shadow follows the masked texture's actual alpha and falls only on the already-composited lower terrain layers, preserving their alpha coverage exactly. It cannot fill transparent holes or extend an island silhouette, does not land on the reference guide, and is excluded with hidden or fully transparent casting layers. Layer opacity attenuates shadow strength. Canvas, Preview and baked PNG use the same composition; shadow edits are part of the recipe and ordinary undo/redo.

The fourth painter page, Preview, inspects tile seams in a shared grid using the current painting and selected project Assets. Its persisted `preview` setup has 1–8 rows and columns and exactly one cell per grid position, containing the current painting, an image reference, or an empty cell. Preview uses original source resolution; the guide/reference background is excluded from the painting tiles and baked output.

Painting create/update/delete shares the project repository semaphore and current-tree lock. Writes require the mounted project revision plus the last-read painting freshness token; create requires an absent painting. Recipe-only saves do not change gameplay-source revision. An explicit bake saves the recipe, `assets/<id>.png`, and the advanced project revision in one recoverable transaction. The painter home Bake All Assets command rebuilds recipes with bound outputs in source-dependency order and commits every recipe, generated PNG, and the one project revision together. Native admission guards all freshness tokens, unique output ownership, and referenced source availability against the final resource set before mutation. Any render, admission, or publication failure leaves the prior complete batch intact; unbound drafts are skipped. A bake rejects an existing unrelated resource ID; subsequent bakes can replace the recipe's recorded output. Deleting a recipe preserves its baked asset. Portable JSON export includes recipes; gameplay compilation and Arkpack include only the baked PNG. Undo/redo and transient tool/view state belong to the painting session and are not serialized as history. The session commits completed gestures synchronously, independently of React rendering, and survives tab changes. React subscribes to document and tool projections; the disposable native canvas owns pointer capture, pending gestures, cursor/view geometry, rendering frames and debounced shadows. Detaching the canvas cancels unfinished gestures and releases its input/render resources without changing the committed draft or history. While a layer changes, its last computed shadow remains visible through the gesture and debounce interval; the recomputed shadow replaces it on settlement. Painter canvas diagnostics are automatically batched into the application diagnostic log with an attachment session ID and ordered event sequence. They correlate native input, accepted/cancelled gestures, document revisions, decoder generations and completed render frames without recording source image bytes. Capture diagnostics record painter-owned set/release calls and reasons, window capture-phase input targets, stable per-attachment node identities, surface/ancestor removal and reinsertion, focus, visibility and pointer-lock changes. DOM records describe delivery-time state and retain queued removals even when the node is already reconnected; they do not read DOM text or form values. They observe capture routing without changing gesture decisions or patching browser APIs.
