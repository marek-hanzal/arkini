...
- The standalone Effect tick-engine idea is captured in `v0-effect-tick-engine-2026-06-17.md`: evaluate `(GameConfig, save, action/tick, nowMs)` outside React, emit domain events, keep UI as subscriber/adapter, and test with fake time/fake services. New engine work must be real Effect-first code under `src/v0/game/engine/fx/*`, not `logic/*` plus a cosmetic wrapper; use the typed error channel for rejected actions/config problems. `src/v0/game/engine/logic/*` was intentionally removed. Do not reintroduce pure domain helpers there; split logic into focused Fx files instead.
- Engine Fx file hygiene is strict: one exported function per file, file name equals function name, and function input shape lives in `namespace FunctionName { export interface Props { ... } }`. Avoid monoliths like old `applyGameActionFx.ts`/`runGameTick.ts`; dispatch files should dispatch, flow files should orchestrate one flow, and helper Fx files should do one domain operation.
- Engine randomness must use `RandomServiceFx` (provided by `runEffect` in app code and fake services in tests). Do not add seeded local RNG helpers such as `nextGameRandom`; that bypasses the existing service boundary and turns deterministic tests into folk magic. Initial `GameSave` can still be bootstrapped from `GameConfig.startingState` until persistence exists.
...
1. validated/compiled `GameConfig` defines all static gameplay data and IDs.
2. `RuntimeGameEngineAdapter` owns the current `(config, save)` pair and is the gameplay source of truth.
3. `GameRuntimeStore` exposes runtime selectors/actions to React through `useSyncExternalStore`.
4. Visual event adapters translate domain events into TileEngine motion requests; React Query is not in the board/inventory gameplay loop.

`TileEngine` is generic tile interaction infrastructure: pointer lifecycle, hit testing, drop target geometry, snap/rollback, handoff and tile motion. It must not know Arkini game rules. Game-specific drop policy belongs in `src/v0/play/drop`; board/inventory adapter wiring reads runtime selectors and dispatches runtime actions through `src/v0/play/runtime`.
...
The tick engine closure order is documented in `v0-tick-engine-integration-readiness-2026-06-17.md`. Runtime selectors/actions are now the live board/inventory path. Keep sealing parity gaps inside the runtime engine/store instead of reviving React Query cache patches or old SQLite gameplay mutations. A concrete proposed action should be validated by an Effect-first readiness entrypoint that reuses the same focused validation Fx as `applyGameActionFx`; React/UI must not duplicate gameplay rules just to decide whether an action looks available.
...
Action visual events may carry `animation` metadata. Treat this as the data contract between domain events, runtime visual adapters and TileEngine rendering:
...
- `mode: "sequence"` means visual event adapters/TileEngine motion may stagger presentation by `delayMs` / `sequenceIndex`. Stash exhaust output uses this to drip items one by one.
...
Bug reports are boring JSON on purpose: browser metadata, active sheet/error context, runtime revision/board/inventory summary, last loaded scenario, TileEngine DOM snapshots, database/query diagnostics and the latest timeline entries. The timeline records TileEngine pointer/drag/drop/hover feedback/motion lifecycle, runtime action results, dev scenario loads and visual-event motion registration.
...
TileEngine slot and actor components use adapter-owned `renderKey` tokens to avoid waking memoized cells/actors just because runtime selectors produced a fresh view object. If a TileEngine adapter puts scalar renderer data directly in `slot.data` or `tile.data`, include every relevant scalar in `renderKey`. If data identity itself is meaningful, omit `renderKey` and the engine falls back to object identity. Do not use `renderKey` to hide geometry, visibility, style, motion or feedback changes; those are compared separately and must keep rendering observable.

Inventory slot data should stay layout-only (`slotIndex`). Stack/item/quantity belongs on the tile data, where real stack changes can wake the actor and renderer. Board item actors can use stable board-item keys because the actual mutable board item details are read by focused tile views. TileEngine keeps the latest drag config behind a stable ref; pointer-down re-reads the binding from that ref, so runtime state changes can update drag/drop behavior without making every actor prop change. This keeps drag/hover/board churn from politely asking every actor to rerender because one twig blinked.

## Active improvement priorities

1. Keep the current checkpoint stable while the runtime engine becomes the only gameplay source of truth. Avoid starting `009-economy-content-pass` or another broad refactor until this runtime parity pass settles.
2. Keep runtime selectors/actions as the board/inventory path. Do not reintroduce React Query cache patching for gameplay state.
3. Keep action visual events explicit through `ActionVisualAnimation`; test event ordering and animation contract when adding new animated behavior. Drop runtime animation flags must survive error wrapping; losing `parallel-swap` breaks swap concurrency before visual events even get a vote.
4. Add more Vitest coverage around domain action results, visual-event ordering, runtime drop actions, placement planning and manifest validation when the behavior is being touched anyway.
5. Keep board/inventory surfaces as render shells; put TileEngine model wiring in concrete adapter hooks. Adapter hooks may wire runtime selectors/actions and feedback, but gameplay decisions such as tap activation intent, drop action policy, merge resolution and inventory statefulness should be delegated to small pure `logic/` helpers or play/drop action resolvers.
...
Producer output also uses explicit `mode: "sequence"` spawn events. First item enters immediately, later items follow by `actionVisualSequenceDelayMs`; do not silently revert producers to one instant visual batch unless we deliberately want the player to miss the output animation.

Board merge drops use TileEngine `parallel-merge`, which dispatches the runtime merge immediately instead of running a pre-commit source snap first. Merge fade/pop belongs to the merge visual event group, not to a separate “please wait while this twig politely parks itself” phase.
