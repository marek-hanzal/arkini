...
| --- | --- | --- |
| `game-event` | Internal transition receipts, strict downstream event vocabulary and final projection | [`../game-event/fx/projectCommittedEngineFactsFx.ts`](../game-event/fx/projectCommittedEngineFactsFx.ts) |
...
### Authored template initialization and space dimensions

New-game start applies `start.spaces` references through [`applyBoardTemplateRuntimeFx`](../board-template/fx/applyBoardTemplateRuntimeFx.ts), the same explicit-snapshot operation used by [`applyBoardTemplateFx`](../board-template/fx/applyBoardTemplateFx.ts) to atomically replace the current board. It destroys that space’s physical roots and their complete input/job ownership trees, including queues, deliveries and selected lines, without refunds or outputs. Rooms owned by removed identities are recursively destroyed; other surviving spaces retain their exact state. Autofill only admits material from the producer’s home space. If a producer travelled after admission, a surviving remote delivery targeting the reset owner returns to its leased home cell.

[`readPhysicalRootOriginFn`](../item-location/fn/readPhysicalRootOriginFn.ts) identifies the home origin of Board, Delivery and departed Terminal roots; internally owned items have no origin of their own. [`readRuntimeOwnershipClosureFn`](fn/readRuntimeOwnershipClosureFn.ts) follows Item-to-input, Item-to-Job and Job-to-material edges from the chosen roots. Template reset and Inventory destruction share that traversal while retaining their separate space-selection and cleanup policies.

Runtime and State store `templateUidBySpace`, never dimensions. [`readBoardSizeFn`](fn/readBoardSizeFn.ts) resolves each active UID from loaded configuration, with project dimensions for unmapped spaces. Start assignments do not override saved active identities. Hydration never re-expands template contents or compares existing coordinates with current geometry. New placement admission, scene and camera consume the active template dimensions; aggregate checks retain structural identity, occupancy and job/unit integrity.

### Item-owned Inventories

Inventory is the shorter name for an item-owned numeric Space, not a separate storage primitive.

[`space/resolveInventoryFx`](../space/fx/resolveInventoryFx.ts) owns lazy allocation and initialization in the caller's unpublished draft. Runtime Item and State Item carry an `inventories` record from creating template UID to numeric Space; existing `templateUidBySpace` owns active geometry and may change independently. There is no separate ownership registry. Allocation reserves every authored literal and occupied current/snapshot address. Outcome application passes the exact definition owner; receiver transport passes its target. A dead owner cannot create an Inventory Space. On one owner, outcomes and receiver transport referencing the same template reuse one room regardless of order; distinct template UIDs receive distinct rooms. Same-ID replacement explicitly preserves the bindings.

[`destroyInventoriesFx`](../space/fx/destroyInventoriesFx.ts) integrates with permanent identity removal, passive subtree discard and template replacement. It destroys room contents, complete input/job trees, deliveries originating there and nested owned rooms, without lifecycle settlement or refunds. Detached-job callers pass their pre-settlement ancestry explicitly. Captured input, unit, job and outcome work stops when its identity or room was destroyed. Destruction removes active-template metadata, clears invalid history, and falls back from a deleted current room to surviving history or configured start. [`isSpaceRetainedFn`](../space/fn/isSpaceRetainedFn.ts) compares owner identity across snapshots so recycled numbers cannot retain old navigation references.

[`checkInventoriesFn`](../space/fn/checkInventoriesFn.ts) checks unique bindings, separation from authored addresses and existing template initialization at commit and hydration. Shape changes affect persistence, destination authoring, template-reference validation, graph recipe projections and generated project schema. Ordinary placement geometry, global switches and unrelated fixed spaces retain their existing owners.
