...
- An unbounded automatic Clock with one potentially enabled Clock line uses the larger of interval and runtime as its optimistic action cadence. Permanently disabled lines do not enter that pool. Multiple potentially enabled Clock lines return `partial`: the analysis does not model their exclusive shared pulses, correlated outputs across alternatives, or rule-dependent eligibility. Multiplying each line’s interval by inverse weight would invent incorrect independent timing when alternatives produce the same item or supply each other’s required co-products. Initial impulse delay, queue contention and conditional runtime changes remain outside the single-line lower bound; interactive production retains ordinary line timing.
