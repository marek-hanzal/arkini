...
- When the complete canvas scene exceeds its viewport on an axis, hovering near that viewport edge pans the shared camera, bounded to one scaled world cell beyond the scene. Board, Toolbar and Inventory all activate edge navigation, including the gaps between surfaces. Held pointer buttons, overlays, pointer exit, blur, resize and teardown stop it; demand frames run only while the camera moves.
