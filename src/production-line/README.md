...
[`enqueueLineRuntimeFx.ts`](../production-job/fx/enqueueLineRuntimeFx.ts) requires the initiating command's expected trigger: player commands use `manual`, Clock pulses use `clock-interval`, and item termination uses `item-termination`. [`isLineAdmissionOpenFn.ts`](fn/isLineAdmissionOpenFn.ts) owns the live owner-state boundary for accepted intent. [`readJobOwnerExitFn.ts`](../item-terminal/fn/readJobOwnerExitFn.ts) resolves owner removal when terminal work or a self-depleting job completes. Job settlement consumes that decision without selecting Clock lines or interpreting timer mode.
