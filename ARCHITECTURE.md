...
src/tile-motion → { src/tile-presentation, exact retained Pixi actor/animation/surface capabilities, src/application-runtime RendererRuntime }
retained Pixi scene/delivery → src/tile-interaction → { src/item-interaction, src/tile-motion, src/tile-presentation, exact retained Pixi capability leaves }
...
src/production-line-authoring/ui → { exact production/item schema leaves, src/authoring-session/ui, reusable src/ui }
...
- `src/tile-motion` owns retained actor-motion services and contracts, bounded cue-lane scheduling, quantity-delay policy, magnetic response, target chasing, spawn/stack/input/swap choreography, presentation synchronization, and cleanup. Its lifecycle Effects remain separate under `fx/`; sole-owner interaction, actor-claim, lane-claim, and quantity-projection helpers stay private with their public owner. Tile Motion consumes Tile Presentation and exact retained Pixi actor, animation, surface, and renderer-runtime capability leaves. Retained Pixi drag, drop, delivery, and scene composition consume it; the dependency never reverses from those execution owners into Tile Presentation, and Tile Motion owns no React, routes, Electron, or gameplay mutation.
- `src/tile-interaction` owns retained pointer gestures, activation intent, drag/drop preview and settlement, mounted gameplay-command Atoms, magnetic coordination, and route-local interaction cancellation. Main and Inventory surfaces implement its exact readonly capability contracts; concrete Pixi scene, delivery, and provider composition consume the capabilities. Tile Interaction may call Item Interaction and Tile Motion plus exact actor, animation, runtime, and layout leaves, but never imports concrete Pixi scene owners. Primitive gesture helpers stay private in their controller factory.
...
- `src/ui` owns cross-product primitives and retained React/Pixi scene execution, including generic fact layout and compact duration formatting. Product-specific presentation policy remains with its top-level product owner. `src/@routes` owns registration, loaders, redirects, route context, and route-specific composition; routes may share only explicitly ignored `-*` route-private helpers, never import another route module.
...
Tile Presentation projects exact Runtime and committed-event facts into actor values, feedback, replacements, and motion intents. Tile Motion turns those facts into bounded lanes, magnetic response, target chasing, actor claims, and spawn/stack/input/swap playback while consuming only exact retained Pixi capabilities. Tile Interaction owns pointer gesture, activation, drag/drop execution, command admission, and cancellation against owner-defined surface capabilities. Pixi owns retained Board, Toolbar, and Inventory scene composition: display objects, geometry, hit testing, z-order, and demand rendering. Motion is the only interpolation clock. Item Interaction decides ordinary click actions and every drop outcome. Main and Inventory canvases have separate actor stores; their handoff carries presentation geometry only and cannot assert runtime identity continuity. See the local [`src/ui/pixi/README.md`](src/ui/pixi/README.md).
