...
- Info is the first completed capability vertical slice. Clicking Info freezes the exact runtime ID and originating actor, closes hover, and opens one Canvas-local viewport-constrained `TileWorkspace` over an unchanged Board/Toolbar. The workspace owns one generation-safe `closed | entering | open | exiting` lifecycle, backdrop dismissal, capture-phase Escape ownership, focus containment/restoration, stale-target close, and local Motion only. The Info projection exposes only title, category subtitle, description, and resolved authored source/composite resource URLs. Its sparse body reserves most of the stable modal footprint for a large inspectable item visual and gives long descriptions internal scrolling. Exact item removal closes the workspace; slot replacement never retargets it.
