...
- Product lines own their normal activation `inputs` and `output` inline. `inputRefId` and `outputTableId` are only legacy/shared escape hatches; do not create one-off top-level input/loot definitions for product-specific data.
...
- Job/event split: anything delayed, scheduled, retrying, blocked, or persisted for future processing is a job. `GameEvent` is only an output for something processed now. Producer queues are FIFO, including blocked deliveries. Runtime dispatch/readiness catches up ready ticks before evaluating player actions. Spawn jobs with unresolved dependencies are not wakeable until dependencies resolve, and dependency cycles are invalid save data. Stash dumps preflight atomically, count the removing stash cell as available output space, and then emit output through immediate item spawn jobs whose sequence metadata drives visual spawn order.
