...
- Era X starts prestige construction materials: Dye Workshop now produces Pigment from Vegetables + Water and Luxury Cloth consumes Pigment; University issues Glazier Workshop blueprint; Glazier Workshop requires Glassworks proximity and produces Stained Glass from Glass + Pigment + Charcoal + Water. Because it uses Charcoal, it requires nearby Purifier and emits random Pollution. Stained Glass is the current Era X master/prestige construction output.
- Marble discussion: defer it. When added, prefer a Stonemason/Quarry tier path (Prospector Guild higher tier finds Marble Deposit, Quarry/Marble Quarry extracts Marble, Stonemason T2 finishes Marble Blocks) instead of creating unrelated one-off buildings for every fancy rock. Keep this as a future luxury construction branch, not part of the current stained-glass pass.
