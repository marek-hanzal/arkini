...
Scheduled owners filter interval Clock lines by rules, draw by `clockWeight`, and enqueue only when inputs are available in the line or on the Board. At expiry a Board owner draws one eligible lifetime line by weight and enqueues it with the same input, runtime and output settlement as an ordinary Job. An item held inside another job, input or delivery cannot own a Board Job: expiry uses the same weighted selection but immediately settles the selected line outcome, skipping its inputs and runtime. `Item.clock` composes scheduling data; `item-schedule` owns phase, lifetime and the Clock override, while Production retains queue ordering and the complete job/delivery lifecycle. An exhausted schedule closes ordinary new intent; accepted runnable work still dispatches normally in loose-kill mode, while kill-switch cancels it before the selected lifetime Job. Player-control admission is separate from autonomous work and shared by production commands and their projections.

[`isLineAdmissionOpenFn.ts`](fn/isLineAdmissionOpenFn.ts) owns the authored-line admission boundary for ordinary and lifetime intent. [`readScheduledLineOwnerExitFn.ts`](../item-schedule/fn/readScheduledLineOwnerExitFn.ts) resolves the terminal owner policy at completion. Job queueing and settlement consume those decisions without selecting Clock lines or interpreting timer mode.
...
Clock kill-switch cancels earlier owner intent and active work without refunding consumed inputs or emitting their line outputs. A selected lifetime line then runs as an ordinary Job. Its completion removes the owner and discards any output or returned input that cannot fit; loose-kill instead keeps a blocked completion ready for retry. Without an eligible lifetime line, kill-switch uses [`forceRemoveRuntimeItemFx`](../game-runtime/fx/forceRemoveRuntimeItemFx.ts) to remove the owner immediately. Successful completions at the expiry boundary still win.
