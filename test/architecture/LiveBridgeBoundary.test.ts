...
describe("live game bridge boundary", () => {
	...
	it("keeps one root owner across routes, HMR handoff, and controlled Electron close", () => {
		...
		expect(provider).toContain("previousHotShutdown");
		expect(provider).toContain("Effect.tryPromise");
		expect(provider).toContain("Effect.zipRight");
		expect(provider).toContain("RendererRuntime.runPromise");
		...
		expect(shell).toContain("owner.replaceFx(packageId)");
		expect(shell).toContain("state.saveRecoveryKey");
		expect(shell).toContain("owner.clearFailedSaveAndRetryFx()");
