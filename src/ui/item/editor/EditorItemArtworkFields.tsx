				...
				{(sourcesField) => (
					<div className="grid gap-3 border-t border-line pt-4">
						<div className="flex items-center justify-between gap-3">
							<div>
								<h3 className="text-sm font-semibold">Progress assets</h3>
								<p className="mt-1 text-xs text-muted">
									Ordered single-image states shown after the default composition.
								</p>
							</div>
						...
						{(sourcesField.state.value ?? []).map((_, index) => (
							...
							>
								<group.AppField name={`sources[${index}]`}>
									{(field) => (
										<field.AssetField label={`Progress asset ${index + 1}`} />
									)}
								</group.AppField>
								<Button
									className="self-end"
