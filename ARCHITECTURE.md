...
Tile interaction uses one Canvas-wide actor system shared by the rendered Board, Inventory, and Toolbar. The Inventory is a non-modal Canvas-local surface and the optional Toolbar is one passive row; all three expose stable slot anchors only. `TileActorLayer` renders exactly one DOM actor for each live runtime item identity, independent of its current storage surface, so Board ↔ Inventory ↔ Toolbar movement never creates a clone. Arkini owns actor phase, operation generation, source/target semantics, z-order, topmost `slot | surface | outside` hit testing, engine command selection, and current placement. Motion owns only the direct pointer drag values on that actor. Stable placement and hover/target feedback update immediately; release resets the local drag offset, invokes one public atomic engine drop command, and clears the exact pending generation when that command finishes. The UI never infers move, merge, consume, swap, stacking, or storage eligibility. There is deliberately no tile movement interpolation, crowd displacement, producer-emission choreography, scale/fade effect, retained removed identity, or post-drop presentation lifecycle. The ordered semantic event stream remains independent of this presentation baseline and is the source for future choreography. No ghost, clone, screenshot, hidden canonical tile, per-surface actor tree, manual RAF drag runtime, or direct WAAPI path exists.
