...
- Choose The Path is represented by three Era XI keystone building items: `producer:house-of-engineers`, `producer:cathedral`, and `producer:mage-lodge`. University issues their blueprints after prestige/treasure inputs. Keystone buildings emit passive global effects that block counter-path blueprint product lines and counter-path item creation through `line.blockStart` + `item.blockCreate`. Later branch products should be baseline-hidden and revealed by path effects for branch-tagged variants.
- Item detail UI must list generated effects from `passiveEffectIds`, especially buildings/keystones, so hard gameplay rules are visible instead of occult nonsense.
...
- Job/event split: anything delayed, scheduled, retrying, blocked, or persisted for future processing is a job. `GameEvent` is only an output for something processed now. Producer queues are FIFO, including blocked deliveries. Stash dumps preflight atomically and then emit output through item spawn jobs immediately.
