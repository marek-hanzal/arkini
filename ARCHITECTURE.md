...
src/item-line-detail/read → { src/game-runtime, exact Item Definition/Location and production reads, remaining Engine query/Sources leaves }
src/item-line-detail/ui → { Item Line Detail reads, src/item-detail-frame, exact production commands/status, src/renderer, reusable src/ui }
src/item-detail-frame → { remaining exact Item Detail reads and gameplay schemas, src/application-diagnostics, src/renderer, reusable src/ui }
...
- `src/item-line-detail` is the precise vertical owner for Item Lines without becoming an Item Detail superdomain. Its framework-neutral `read/` subtree owns the line-detail contracts and canonical board/stored projections, input state, material-autofill availability, output sets, roll branches, active-job facts, selection identity, and cross-space semantics. Its `ui/` subtree owns renderer projection, Item Lines/row/summary/input/output presentation, search/filter state and autofocus. Actual enqueue/default/withdraw mutations remain in Production and are admitted and settled through Item Detail Frame; Queue, Sources, dialog/tab composition, general overview/definition information, and ordinary primary actions remain with their exact owners.
