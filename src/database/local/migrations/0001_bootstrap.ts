...
export const migration0001Bootstrap: Migration = {
	async up(db: Kysely<unknown>) {
		...
		await db.schema
			...
			.addUniqueConstraint("boardItem_saveGameId_x_y", [
				"saveGameId",
				"x",
				"y",
			])
		...
		await db.schema
			...
			.addUniqueConstraint("inventoryStack_saveGameId_slotIndex", [
				"saveGameId",
				"slotIndex",
			])
			.execute();
		...
		await db.schema
			.createIndex("boardItem_saveGameId")
			.on("boardItem")
			.column("saveGameId")
			.execute();
		await db.schema
			.createIndex("inventoryStack_saveGameId")
			.on("inventoryStack")
			.column("saveGameId")
			.execute();
	},
