...
**Audience:** a future Codex/ChatGPT reviewer working from a local repository or
an exported snapshot containing `.git`.

**Status:** current review protocol, not backlog, roadmap, or implementation
plan.
...
## 2. Review mode and implementation separation

A review pass is read-only unless the user explicitly couples it to
implementation work.
...
- Record findings on GitHub. Do not preserve a second active review backlog in
  repository-local scaffolding.

When implementation is explicitly requested in the same task:

- stabilize the evidence and ownership verdict before broad edits;
- keep at least one clean-head reviewer read-only;
- separate audit findings from implementation results in the final report;
- do not let an implementation worker approve its own ownership changes;
- reject or roll back a refactor when behavior proof or mental-load improvement
  is inconclusive.
...
Stop if the supplied repository or snapshot does not contain `.git`.
...
3. this root `REVIEW_CODEBOOK.md`;
4. current root contracts:
   - `README.md`
   - `ARCHITECTURE.md`
   - `CODE_GUIDE.md`
...
6. relevant historical notes only when current behavior needs an oracle.
...
### 4.9 Effect Atom renderer boundary

The renderer has one process-lifetime Atom registry/runtime root. `Atom.fn`
owns concrete renderer command admission, pending/result state, and Effect
execution when those facts must be shared with React.

Atoms may project authoritative engine or application state. They may not
...
Prefer one natural command Atom per concrete user operation and narrow read
Atoms/selectors over official services or `SubscriptionRef`. Reject additional
Atom registries, captured private schedulers, generic command factories,
handwritten pending stores beside `Atom.fn`, lifecycle managers, and
callback-injection adapters.
...
For substantial deletion/refactor work, record a before/after score from `0`
(locally obvious) to `2` (requires reconstructing remote invariants) for:
...
Call the result improved only when behavior is preserved, no duplicate owner is
introduced, at least one of authority discovery, mutable truths, or lifecycle
joins improves, and no axis materially worsens.

Measure authored production LOC separately from tests and generated code.
Meaningful deletion is preferred when it removes dead public surface,
duplicated truth, synchronization, or obsolete pipelines. A lower line count is
not success when complexity is merely moved into a generic framework, test
adapter, schema layer, or configuration ceremony.
...
- one renderer Atom registry/runtime exists;
- command pending/result ownership is not duplicated beside `Atom.fn`;
...
A clean review still records the reviewed HEAD, baseline, validation, and clean
verdict in the owning review root. Create a new root only when one does not
already exist.
...
- [ ] one renderer Atom registry/runtime is preserved.
- [ ] command pending/result ownership is not duplicated beside `Atom.fn`.
