...
Electron exposes one canonical project writer rather than a generic file writer. A successful save commits the changed content file together with a monotonically newer `editor.json.updatedAtMs`, rereads the exact post-write project from disk, and returns that authoritative record and revision. Renderer code cannot write `editor.json` directly. Recent-project ordering, persisted content, and the canonical renderer snapshot therefore advance through the same writer.
