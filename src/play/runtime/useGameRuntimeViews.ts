...
import { readInventoryView } from "~/play/runtime/readRuntimeViews";
import { readItemCatalogView } from "~/play/runtime/readRuntimeViews";
import { readItemView } from "~/play/runtime/readRuntimeViews";
...
const sameBoardView = (left: BoardView, right: BoardView) =>
	...
	left.items.every((leftItem, index) => sameBoardViewItem(leftItem, right.items[index] ?? null));
...
export const useGameBoardItem = (boardItemId: string): BoardViewItem | null => {
	...
	return useGameRuntimeSelector(selector, sameBoardViewItem);
};
