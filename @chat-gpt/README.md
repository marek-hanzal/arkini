...
- Heroes Guild I is the current post-goldsmith test branch: Town Hall IV sells its blueprint for coins, guild expeditions consume coins plus food/drink, return tiered locked hero chests, and chests open as keyed stashes. Stash clicks auto-fill open-time inputs from board/inventory when available, board auto-fill animates into the stash through the shared activation-input visual path, and missing inputs open detail instead of surfacing an action error. Goldsmith produces Hero Key III; other hero key tiers are cheat-only acquisition placeholders for now, but all four key tiers have dedicated key art.
