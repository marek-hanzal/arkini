...
- Clock lines remain ordinary authored Flow relations, and finite Clock `onExpire` uses the shared expiry projection. Lines on `ui: simple` owners without both an interval and the Clock flag, and permanently disabled automatic schedules, have no Estimate execution route.
