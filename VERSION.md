...
Load and `arkini-cli arkpack verify <file>` classify the single file offline. Verification checks payload digest/signature, Fulcio chain and certificate-transparency proof, Rekor proof, issuer, and exact repository workflow identity against the embedded [`src/arkpack/artifact/trusted-root.json`](src/arkpack/artifact/trusted-root.json). The certificate's workflow ref and the application's version are not channel identity. Failure becomes Community, never a load rejection.
