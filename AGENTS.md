...
- Primitive `*Fn` helpers are private and co-located with their exact owner by default. Prefer one cohesive linear owner over a constellation of tiny exported modules and import hops; file length alone is not a split rule. A standalone exported `fooFn.ts` is justified only for an independently shared policy, algorithm, or calculation with its own public contract. Such files may use the exact owner's local `fn/` directory; that directory contains only those total explicit-input data-to-data operations and their operation-owned declarations, never Effect programs, schemas, standalone types, Context/Layer/error/capability declarations, constants, or a global/shared junk drawer.
...
- Prefer one cohesive public ownership boundary per file. Sole-owner helpers, commands, constants, Props, Result, and other implementation declarations stay private and co-located with that owner. Split only when a policy, lifecycle, capability, or contract is independently meaningful; do not export implementation details merely to keep files short or directly test them. Standalone public schemas, typed failures, Context/Layer/capability identities, Atom state identities, and framework entrypoints retain their own domain-owned files. Do not create catch-all barrels or `types.ts` piles.
