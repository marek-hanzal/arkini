...
- Added Arkini pack v1 binary format:
  ...
  - outer gzip wrapper
- `game:compile` now writes `game/arkini.game.arkpack.gz`.
- Removed generated `game/arkini.game.json` and `game/arkini.assets.json` from active runtime artifacts.
- Browser runtime loads the pack through `loadDefaultGameConfig`, fetches the `.arkpack.gz`, inflates it with `DecompressionStream("gzip")`, decodes MessagePack, creates Blob URLs for PNG resources, validates with `GameConfigSchema`, then hands the normal `GameConfig` to existing runtime creation.
- Node/test/CLI paths use `loadGameConfigPackFromFile`, backed by Node `zlib.gunzip`.
- `game:validate` accepts either source packages such as `game/arkini` or a single `.arkpack.gz` file.
- `audit:current` now rejects old split compiled JSON artifacts and validates the committed binary pack.
...
- `npm run game:validate -- game/arkini.game.arkpack.gz`
