...
Double-tapping an inventory stack in that mode dispatches `inventory.item.place` with `placementMode: "nearest_by_manhattan"`, the selected seed cell and `quantity: 1`. The engine consumes one item from the stack and reuses the shared board-then-inventory placement planner, so the spawned item tries the seed cell first and falls back by Manhattan distance around that seed.
...
- Inventory UI only changes double-tap behavior in placement mode: double-tap places one item from the selected stack around the seed and keeps the sheet open. Single tap does nothing useful here and must not trigger placement.
