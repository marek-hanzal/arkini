...
- Durable item storage is normalized through one `itemInstance` table. Board items, inventory stacks, and producer/stash input storage are projections over the same saved item entity, with `locationKind` deciding whether the instance is active on the board, passive in inventory, or owned by another item as activation input storage. The old physical split between `boardItem`, `inventoryStack`, and JSON-stashed producer inputs is gone from runtime code because making one item pretend to be three database species was cute only in the same way a car fire is warm.
...
- Producers and stashes can hold activation input storage. Dropping an accepted item onto one stores it as a nested `itemInstance` row up to the configured capacity; item detail shows stored/needed inputs and can withdraw them board-first with inventory fallback. Activation consumes configured inputs only after the complete rolled output placement plan succeeds.
...
src/activation/                 Activatable item runtime: producer/stash activation, output rolling, depletion, and nested input storage.
