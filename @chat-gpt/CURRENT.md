...
- Game shell presentation #332 is CSS-only and geometry-neutral: the canonical `GameShell` owns a light/dark semantic base gradient plus one pointer-transparent pseudo-element that crossfades a second static gradient composition over 48 seconds; reduced-motion freezes that layer. `BoardFrame` owns the larger layered surface shadow, and the Toolbar scroll owner owns a smaller contact shadow so vertical overflow clipping cannot erase it. No decorative DOM, moving gradient coordinates, slot shadow, layout measurement, or gameplay stacking owner was added.
...
> Continue #317 incrementally from the shared compact tile chrome plus the landed Info and initial Status slices. Treat the current broad Info fact set as deliberately additive: remove low-value fields after native review instead of hiding potential player-facing facts before they can be judged. Status still deliberately omits line-level readiness/progress/queue/output blockage and currently impossible depleted-idle runtime state. The next large agreed UI slice is likely Lines; extend the same finite capability availability and shared `TileWorkspace` host, keep capability-specific projections independent, and do not resurrect a universal `TileDetail`, duplicate modal shells, or infer gameplay truth in React. Preserve exact runtime identity, Canvas-local overlay/input ownership, Floating UI as hover positioning only, Arkpack-owned resources, and the one-live-subscription/no-second-truth contract. Selection and shortcuts remain later polish unless the user explicitly pulls them forward.
