...
> `game/archive` no longer contains configuration definitions. All canonical items and the initial game state have been migrated to `game/arkini`; all shared PNG assets belong to `game/arkini/assets`. The prose below is historical gameplay context only.

Current JSON backlog: empty. Everything described below is historical context only.

This folder preserves the historical v0 design notes after completion of the definition migration.
...
Gameplay items live in `game.json` for global utility entries and in `era-*.json` files for era content. If a tile can produce, store, craft, merge, publish grants, or be removed by a tool, that capability is embedded directly on the same item definition. Source items usually omit derived `assetIds`: the compiler derives `["asset:<itemId>"]` from the item id and synthesizes conventional assets. Item `code` no longer exists; the item id is the single stable identity. Asset overrides live in `game.json` only for intentional resource reuse; generated PNG resources still come from `assets/`. Asset overrides do not carry `kind`; asset ownership and rendering are implied by the item `assetIds`, asset id, render mode, and resource id. Craft/blueprint items may list multiple ordered asset ids; runtime maps delivered requirement progress evenly across that array. Craft uses the same producer-style `output` model as product lines, including multi-output rewards, chance drops, effects, and board-first placement/stacking. Source blueprints may omit boring derived guaranteed output when the target item id is already implied by the blueprint id. One owner id is enough; making the config repeat itself was paperwork wearing fake architecture glasses. Omit boring line names when they equal the primary output item name and omit quantity `1` in source inputs/outputs.
