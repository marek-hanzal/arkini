...
- Product lines support `showIf`, an OR list of marker item IDs. Hidden lines are filtered out of runtime views and cannot be set default, fed, or started until any marker exists anywhere in the save. This is the runtime/config bridge for Choose The Path variants.
- Choose The Path plan: after neutral prestige cleanup, player chooses one exclusive starter building: House of Engineers, Cathedral, or Mage Lodge. Engineers/Faith/Mages branch outputs can reuse the same buildings by defining same-output product lines with different inputs and `showIf` markers.
