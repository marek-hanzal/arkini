...
const readTierReports = ({
	...
	Effect.gen(function* () {
		const tiers: PlannerCoverageTierAuditTier[] = [];
		for (const tier of template.tiers) {
			...
			const attemptSummary = yield* readPlannerCoverageAuditSummaryFx(attemptedResults);
			const cumulativeOutcomes =
				yield* readPlannerCoverageAuditOutcomeCountsFx(cumulativeResults);
...
export const mergePlannerCoverageTierAuditReportsFx = Effect.fn(
	"mergePlannerCoverageTierAuditReportsFx",
)((reports: ReadonlyArray<PlannerCoverageTierAuditReport>) =>
	Effect.gen(function* () {
		const { finalResults, items, template } = yield* Effect.try({
			catch: (cause) =>
			...
			try: () => {
				...
				for (const report of reports) {
					...
					for (const sourceItem of report.items) {
						...
						if (itemById.has(item.itemId))
							return fail(
								`Planner coverage shard item is duplicated: ${item.itemId}.`,
							);
				...
				const finalResults = items.map(({ attempts }) => {
					...
					if (attempt === undefined)
						return fail(
							"Planner coverage shard item unexpectedly lost its final attempt.",
						);
		...
		const tiers = yield* readTierReports({
			items,
			template,
		});
		const finalOutcomes = yield* readPlannerCoverageAuditOutcomeCountsFx(finalResults);
		const saturatedTier = tiers.find(
