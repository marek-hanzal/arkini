...
- the Editor Board runs the same canonical session commands and routed Board, Toolbar, Inventory, Item Detail, audio, cheat spotlight, and Pixi UI as installed gameplay. Its process-owned lifecycle has no autosave and serially discards the old session before publishing a changed project revision or restoring a scenario. `Cmd+P` is always admitted there through canonical Cheat mode. Named scenarios are explicit project-owned SQLite snapshots of `StateSchema`: strict same-major restore deletes only a proven-invalid scenario, while every major project save atomically deletes all scenarios and minor saves preserve them.
