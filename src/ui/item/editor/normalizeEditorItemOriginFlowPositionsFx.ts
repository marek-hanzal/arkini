...
import type { EditorItemOriginFlowPlacedNode } from "~/ui/item/editor/placeEditorItemOriginFlowFx";
import type { EditorItemOriginFlowLayoutProfile } from "~/ui/item/editor/readEditorItemOriginFlowTopologyFx";

interface MutableNodePosition extends EditorItemOriginFlowPlacedNode {
	x: number;
	y: number;
}
...
export const normalizeEditorItemOriginFlowPositionsFx = Effect.fn(
	"normalizeEditorItemOriginFlowPositionsFx",
)(
	(
		placed: ReadonlyArray<EditorItemOriginFlowPlacedNode>,
		profiles: ReadonlyMap<string, EditorItemOriginFlowLayoutProfile>,
		...
		Effect.sync(() => {
			...
			if (relaxed.length === 0) return new Map<string, EditorItemOriginFlowLayoutNode>();
