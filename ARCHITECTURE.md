...
- `GameRuntimeLayerFx` builds config plus one `SubscriptionRef<CommittedTransition>` and the `RuntimeFx`/`CommittedTransitionsFx` projections of that exact store. It can create a fresh Runtime or accept an already hydrated Runtime value without knowing persisted State. `GameSessionLayerFx` chooses fresh versus State hydration outside Game Runtime, then composes runtime → Game Tick → scoped loop. Deterministic tests compose the same runtime and Game Tick layers from `test/support`; there is no production test façade.
