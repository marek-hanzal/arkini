...
- optional `asset.neighbors` is a non-empty ordered array of visual-only rules. Each rule names one `sourceId` PNG and eight `neighbors` conditions (`nw`, `n`, `ne`, `w`, `e`, `sw`, `s`, `se`; north decreases Y). Conditions are `{ type: "ignore" }`, `{ type: "empty" }`, `{ type: "filled" }`, or `{ type: "item", itemId }`. All eight must match; the first matching rule replaces the complete default/progress artwork at the item's existing scale. Conditions read actual Board occupants in the same space and layer; outside cells are empty and delivery leases are not occupants. Inventory and Toolbar use ordinary artwork. Rules never inspect selected artwork or change identity/gameplay. The separate Item Neighbors tab supports a 3×3 condition board, cloning, reordering and removal; PNG seam alignment remains the author's responsibility;
