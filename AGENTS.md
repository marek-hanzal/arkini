# Serakki agent contract

Serakki is an offline, data-driven economy game and Editor. Work as a senior engineer and architect: prefer the smallest explicit design, keep ownership visible, and optimize code and documentation for future LLM comprehension. Engine code is the reference style.

## Operating rules

- Questions and analysis requests are read-only. Edit only on explicit implementation intent.
- Use current source and tests as evidence. Active global contracts are [`README.md`](README.md), [`ARCHITECTURE.md`](ARCHITECTURE.md), [`GAME.MD`](GAME.MD), [`CONFIG.md`](CONFIG.md), and [`VERSION.md`](VERSION.md); GitHub Issues own backlog.
  Read the smallest applicable contract, not the whole set by default. [`DOMAIN_ATLAS.md`](DOMAIN_ATLAS.md) is the on-demand domain index. When a touched domain belongs to a mapped dense island, read that one local README; do not preload unrelated local maps. History is not current authority.
...
- To discover or use the current `serakki-cli`, run `argc preview-cli --build -- --help` once, then drill down with `argc preview-cli -- <subcommand> --help` (for example, `editor import --help`). The `--` passes CLI flags through `argc`; omit `--build` only after that source build. Editor resource imports, including Artwork, Music and SFX, are available through this CLI; find their exact required arguments in help. See [`CONFIG.md`](CONFIG.md) for the resource contract.
...
- Serakki ships desktop UI only. Use fixed desktop layouts; do not add responsive breakpoints or mobile/tablet reflow.
...
- Accessibility is not an Serakki concern. Do not add or preserve ARIA, screen-reader semantics, accessibility-only focus presentation, keyboard-only paths, tests, or review. Pointer, keyboard, and focus behavior is valid only when it is deliberate product UX, never for accessibility compliance.
...
### Generator output and background

- Unless explicitly requested otherwise, first generate exactly one square image at the generator's native output resolution with a true transparent background and native alpha. Request an isolated subject on a transparent canvas with no environment behind it. Never request or accept a checkerboard, transparency grid, gradient, texture, horizon, vignette, floor, or baked background as a substitute for transparency.
- Treat native alpha as successful only when the returned file actually has an alpha channel, transparent corners, a clean complete silhouette, cleared real openings, and no opaque or nearly opaque background residue. Preserve a clean native-alpha result directly; do not composite it onto a key color and extract it again.
- Only when native transparency is unavailable or visibly unusable, regenerate the candidate on a flat, visually uniform chroma-key green background targeting `#00FF00`. A small color deviation or minor variation is acceptable only when the key field remains clearly distinct from every legitimate foreground color and can be removed safely into a clean alpha channel.
- The fallback key background is a temporary technical surface, not part of the art. It must reach every canvas edge, stay unlit, contain no cast or contact shadow, and produce no green bounce, reflection, outline, or spill on the subject. Do not use the background key color or a confusingly similar chroma green anywhere in the foreground; natural grass uses its normal yellow- and olive-green range instead.
- Keep contact shading inside the object or its terrain footprint. Do not paint a detached shadow blob onto transparency or a fallback key background.
...
- Domain ownership names meaning and exact dependency direction; it is not access control and top-level domain names do not imply a DAG. Any owner may directly import a well-designed component, pure operation, schema, type, or capability when its contract fits and the concrete dependency points the right way. Prefer that exact reuse over duplication, forwarding APIs, adapters, or wrappers introduced only because the implementation lives in another domain. Split a broad owner into shallow semantic roots when that makes independently useful contracts visible.
...
- Every production Serakki-owned named callable value makes its execution representation explicit. A callable Props/Output/capability field, function-valued parameter, destructured binding, or local callback ends in `Fn` when invocation returns an ordinary JavaScript value, `void`, `Promise`, or `PromiseLike`, and in `Fx` when invocation returns `Effect`. For callable values, `Fn` identifies direct JavaScript invocation and does not assert that an imperative callback is observationally pure. The stricter totality and purity guarantee above applies when the identifier declares a project value operation. React components and `use*` hooks retain framework grammar. JSX/DOM and external-library property or method names keep their prescribed spelling; suffix the Serakki-owned value passed to them. Anonymous inline callbacks have no suffix.
...
- Reusable Serakki capabilities are readonly objects produced by explicit Effect factories. Do not add project-owned classes, constructor-injected repositories, managers, generic services, or adapters. Framework/external constructors remain valid.
...
- Every test protects one named Serakki regression: a high-risk behavior, invariant, failure boundary, or lifecycle contract. If that regression cannot be stated, do not add the test.
...
- Prioritize engine transactions, Tick, jobs, output, placement, merge ownership, persistence, compiler, and Serapack integrity/trust/load/signing; then renderer-process concurrency; UI presentation last.
...
- Use the smallest representative synthetic fixture. Permanent tests must not depend on `game/serakki`, generated official snapshots, or platform packaging.
